...
export interface ProviderPublicConfig extends ProviderConnectionRecord {
  hasSecret: boolean;
}
...
export class ProviderService {
  ...
  private encryptSecret(secret: string): string {
    if (!safeStorage.isEncryptionAvailable()) {
      throw new Error('OS-backed encryption is unavailable; API key was not saved');
    }
    return safeStorage.encryptString(secret).toString('base64');
  }

  private decryptSecret(provider: string): string | null {
    const encrypted = this.database.getProtectedSecret(provider);
    if (!encrypted) return null;
    if (!safeStorage.isEncryptionAvailable()) throw new Error('OS-backed encryption is unavailable');
    return safeStorage.decryptString(Buffer.from(encrypted, 'base64'));
  }

  public list(): ProviderPublicConfig[] {
    return this.database.listProviderConnections().map((record) => ({
      ...record,
      hasSecret: this.database.hasProtectedSecret(record.provider),
    }));
  }
  ...
  public save(input: ProviderSaveInput): ProviderPublicConfig {
    ...
    if (provider === 'ollama') {
      endpoint = (input.endpoint || 'http://127.0.0.1:11434').replace(/\/$/, '');
      if (!isLoopbackUrl(endpoint)) throw new Error('Ollama endpoint must be loopback-only');
    } else if (mode === 'api' && input.endpoint) {
      throw new Error('Custom remote endpoints are blocked for cloud providers');
    }
    ...
    if (mode === 'cli') {
      ...
      const allowedExecutables = new Set(['claude', 'claude.exe', 'gemini', 'gemini.cmd', 'gemini.exe', 'ollama', 'ollama.exe']);
      if (!allowedExecutables.has(executable.toLowerCase())) throw new Error('CLI executable is not allowlisted');
    }
    ...
    if (typeof input.apiKey === 'string' && input.apiKey.trim()) {
      ...
      this.database.setProtectedSecret(provider, this.encryptSecret(input.apiKey.trim()));
    }
  ...
  public remove(providerValue: unknown) {
    const provider = validateProvider(providerValue);
    this.database.deleteProviderConnection(provider);
    this.database.appendAudit('AI_PROVIDER', 'REMOVE_CONNECTION', `Removed ${provider} connection`, false);
  }
  ...
  public async test(providerValue: unknown): Promise<ProviderResult> {
    ...
    try {
      ...
      this.database.updateProviderTestStatus(provider, false, error);
      this.database.appendAudit('AI_PROVIDER', 'TEST_CONNECTION', `${provider} test failed: ${error}`, true);
  ...
  public async generate(providerValue: unknown, promptValue: unknown, connectionTest = false): Promise<ProviderResult> {
    ...
    const config = this.database.getProviderConnection(provider);
    ...
    try {
      ...
      const error = cleanError(err);
      if (!connectionTest) this.database.appendAudit('AI_PROVIDER', 'INFERENCE', `${provider} inference failed: ${error}`, true);
      return { success: false, provider, error, latencyMs: Date.now() - started };
    }
  }

  private requireSecret(provider: string): string {
    const secret = this.decryptSecret(provider);
    if (!secret) throw new Error(`API key for ${provider} is not configured`);
    return secret;
  }
  ...
  private async callOpenAI(config: ProviderConnectionRecord, prompt: string): Promise<string> {
    const data = await fetchJson('https://api.openai.com/v1/responses', {
      method: 'POST',
      headers: { 'content-type': 'application/json', authorization: `Bearer ${this.requireSecret('openai')}` },
      body: JSON.stringify({ model: config.model, input: prompt, max_output_tokens: 1200 }),
    });
  ...
  private async callAnthropic(config: ProviderConnectionRecord, prompt: string): Promise<string> {
    const data = await fetchJson('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-api-key': this.requireSecret('anthropic'), 'anthropic-version': '2023-06-01' },
  ...
  private async callGemini(config: ProviderConnectionRecord, prompt: string): Promise<string> {
    const key = encodeURIComponent(this.requireSecret('gemini'));
    const model = encodeURIComponent(config.model);
    const data = await fetchJson(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`, {
      method: 'POST',
  ...
  private async callOllama(config: ProviderConnectionRecord, prompt: string): Promise<string> {
    ...
    if (!isLoopbackUrl(endpoint)) throw new Error('Ollama endpoint is not loopback-only');
    ...
    if (!text) throw new Error('Ollama returned no text output');
    return text;
  }
  ...
  private async callCli(provider: ProviderId, config: ProviderConnectionRecord, prompt: string): Promise<string> {
    ...
    let args: string[];
    if (provider === 'claude_cli') args = ['--print', prompt];
    else if (provider === 'gemini_cli') args = ['-p', prompt, '--output-format', 'text'];
    else if (provider === 'ollama_cli') args = ['run', config.model, prompt];
    else throw new Error('Unsupported CLI provider');

    const { stdout, stderr } = await execFileAsync(executable, args, {
      timeout: REQUEST_TIMEOUT_MS,
      maxBuffer: MAX_RESPONSE_CHARS,
      windowsHide: true,
      shell: false,
