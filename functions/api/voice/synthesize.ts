...
const json = (payload: unknown, status = 200) => new Response(JSON.stringify(payload), { status, headers: securityHeaders('application/json; charset=utf-8') });
...
export async function onRequestGet(context: PagesContext): Promise<Response> {
  ...
  return json({
    service: 'mio-voice-v4-synthesis', ready,
...
export async function onRequestPost(context: PagesContext): Promise<Response> {
  ...
  try {
    const upstream = await fetch(context.env.MIO_TTS_ENDPOINT!.trim(), {
      ...
      body: JSON.stringify({
        text: normalized.text, locale: normalized.locale,
        voice: context.env.MIO_TTS_VOICE_ID!.trim(), model: context.env.MIO_TTS_MODEL?.trim() || undefined,
        profile: normalized.profile,
      }),
    });
    if (!upstream.ok) {
      const detail = (await upstream.text().catch(() => '')).slice(0, 500);
      return json({ error: 'Licensed synthesis provider request failed', upstreamStatus: upstream.status, detail }, 502);
    }
    ...
    return new Response(upstream.body, { status: 200, headers: { ...securityHeaders(contentType), 'X-Mio-Voice-Engine': 'v4' } });
  } catch (error) {
    const aborted = controller.signal.aborted;
    return json({ error: aborted ? 'Synthesis request timed out or was cancelled' : 'Synthesis gateway request failed', detail: error instanceof Error ? error.message : 'Unknown upstream error' }, aborted ? 504 : 502);
  } finally {
    clearTimeout(timeout); context.request.signal.removeEventListener('abort', downstreamAbort);
  }
}
