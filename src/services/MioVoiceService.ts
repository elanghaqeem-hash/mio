...
export const MIO_LANGUAGES: MioLanguage[] = [
  { locale: 'auto', label: 'Auto detect', shortLabel: 'AUTO' }, { locale: 'id-ID', label: 'Bahasa Indonesia', shortLabel: 'ID' },
  { locale: 'en-US', label: 'English', shortLabel: 'EN' }, { locale: 'ms-MY', label: 'Bahasa Melayu', shortLabel: 'MS' },
  { locale: 'ja-JP', label: '日本語', shortLabel: 'JA' }, { locale: 'ko-KR', label: '한국어', shortLabel: 'KO' },
  { locale: 'zh-CN', label: '中文', shortLabel: 'ZH' }, { locale: 'th-TH', label: 'ไทย', shortLabel: 'TH' },
  { locale: 'vi-VN', label: 'Tiếng Việt', shortLabel: 'VI' }, { locale: 'es-ES', label: 'Español', shortLabel: 'ES' },
  { locale: 'fr-FR', label: 'Français', shortLabel: 'FR' }, { locale: 'de-DE', label: 'Deutsch', shortLabel: 'DE' },
  { locale: 'it-IT', label: 'Italiano', shortLabel: 'IT' }, { locale: 'pt-BR', label: 'Português', shortLabel: 'PT' },
  { locale: 'nl-NL', label: 'Nederlands', shortLabel: 'NL' }, { locale: 'tr-TR', label: 'Türkçe', shortLabel: 'TR' },
  { locale: 'ru-RU', label: 'Русский', shortLabel: 'RU' }, { locale: 'ar-SA', label: 'العربية', shortLabel: 'AR' },
  { locale: 'hi-IN', label: 'हिन्दी', shortLabel: 'HI' },
];

export const MIO_V2_VOICE: MioVoiceProfile = { id: 'mio-v2-warm-deep', locale: 'id-ID', rate: 0.94, pitch: 0.82, volume: 1, preferredVoiceHints: ['Indonesian', 'Bahasa Indonesia'] };
...
const LATIN_HINTS: Array<[string, RegExp]> = [
  ...
  ['ms-MY', /\b(awak|boleh|daripada|terima kasih|bagaimana|dengan|untuk)\b/i],
  ['es-ES', /\b(el|la|los|las|que|para|gracias|hola|cómo|por favor|usted)\b/i], ['fr-FR', /\b(le|la|les|des|pour|merci|bonjour|comment|vous|avec)\b/i],
  ['de-DE', /\b(der|die|das|und|für|danke|hallo|wie|bitte|nicht)\b/i], ['it-IT', /\b(il|lo|la|gli|per|grazie|ciao|come|voi|non)\b/i],
  ['pt-BR', /\b(o|a|os|as|para|obrigad[oa]|olá|como|você|não)\b/i], ['nl-NL', /\b(de|het|een|voor|dank|hallo|hoe|jij|niet|met)\b/i],
  ['tr-TR', /\b(ve|bir|için|teşekkür|merhaba|nasıl|siz|değil|ile)\b/i], ['vi-VN', /\b(và|của|cho|cảm ơn|xin chào|không|bạn|tôi|với)\b/i],
];

function scoreLatinLocale(text: string): string | null {
  let best: { locale: string; score: number } | null = null;
  for (const [locale, pattern] of LATIN_HINTS) {
...
export function detectMioLocale(text: string, fallback = 'id-ID'): string {
  const value = text.trim(); if (!value) return fallback;
  if (/\p{Script=Arabic}/u.test(value)) return 'ar-SA'; if (/\p{Script=Devanagari}/u.test(value)) return 'hi-IN';
...
function chooseVoice(voices: SpeechSynthesisVoice[], locale: string, profile: MioVoiceProfile) {
  const normalized = locale.toLowerCase(); const language = normalized.split('-')[0];
...
export class MioVoiceService {
  ...
  private state: MioVoiceState = 'IDLE'; private listeners = new Set<MioVoiceListener>(); private preferredLocale = 'auto'; private lastDetectedLocale = 'id-ID';
  ...
  getState() { return this.state; } getPreferredLocale() { return this.preferredLocale; } getLastDetectedLocale() { return this.lastDetectedLocale; }
  setPreferredLocale(locale: string) { this.preferredLocale = MIO_LANGUAGES.some((language) => language.locale === locale) ? locale : 'auto'; if (locale !== 'auto') this.lastDetectedLocale = locale; }
  resolveLocale(text = '') { const fallback = this.lastDetectedLocale || this.profile.locale; const locale = this.preferredLocale === 'auto' ? detectMioLocale(text, fallback) : this.preferredLocale; this.lastDetectedLocale = locale; return locale; }
  ...
  private bindLifecycle(utterance: SpeechSynthesisUtterance) {
    ...
    utterance.onerror = (event) => { this.setState('IDLE'); originalError?.call(utterance, event); }; return utterance;
  }
  ...
  test(locale = this.preferredLocale) {
    const samples: Record<string, string> = { 'id-ID': 'Test, ini Mio V2, salam kenal.', 'en-US': 'Test, this is Mio V2. Nice to meet you.', 'ms-MY': 'Ujian, ini Mio V2. Salam kenal.', 'ja-JP': 'テスト、Mio V2です。よろしくお願いします。', 'ko-KR': '테스트, 미오 V2입니다. 반갑습니다.', 'zh-CN': '测试，这是 Mio V2，很高兴认识你。', 'th-TH': 'ทดสอบ นี่คือ Mio V2 ยินดีที่ได้รู้จัก', 'vi-VN': 'Thử nghiệm, đây là Mio V2. Rất vui được gặp bạn.', 'es-ES': 'Prueba, soy Mio V2. Encantada de conocerte.', 'fr-FR': 'Test, je suis Mio V2. Enchantée.', 'de-DE': 'Test, ich bin Mio V2. Freut mich.', 'it-IT': 'Test, sono Mio V2. Piacere di conoscerti.', 'pt-BR': 'Teste, eu sou a Mio V2. Prazer em conhecer você.', 'nl-NL': 'Test, ik ben Mio V2. Aangenaam.', 'tr-TR': 'Test, ben Mio V2. Tanıştığımıza memnun oldum.', 'ru-RU': 'Тест, я Mio V2. Приятно познакомиться.', 'ar-SA': 'اختبار، أنا ميو V2، سعيدة بلقائك.', 'hi-IN': 'टेस्ट, मैं Mio V2 हूँ। आपसे मिलकर खुशी हुई。' };
    const resolved = locale === 'auto' ? this.lastDetectedLocale : locale; return this.speak(samples[resolved] ?? samples['id-ID'], resolved);
  }
}
