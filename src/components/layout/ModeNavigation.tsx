...
export const ModeNavigation: React.FC<ModeNavigationProps> = ({ activeMode, onSelectMode }) => {
  const items: NavItem[] = [
    ...
    { mode: 'TASKS', label: 'Task Monitor', icon: <ListTodo size={16} />, category: 'System' },
  ...
  return (
    <aside className="w-56 bg-[#090d16] border-r border-gray-800 flex flex-col font-mono text-xs select-none">
      <div className="p-3 text-[10px] text-gray-500 font-bold tracking-wider border-b border-gray-800/80">MODES // WORKSPACES</div>
        ...
        {items.map((item, idx) => {
          ...
          return (
            <React.Fragment key={item.mode}>
              {showCategoryHeader && <div className="pt-3 pb-1 px-2 text-[9px] text-gray-500 font-bold uppercase tracking-wider">{item.category}</div>}
              <button
                onClick={() => onSelectMode(item.mode)}
                className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg font-medium transition cursor-pointer text-left ${isSelected ? 'bg-cyan-950/60 border border-cyan-500/50 text-cyan-300 shadow-sm shadow-cyan-500/20' : 'text-gray-400 hover:bg-gray-800/60 hover:text-gray-200'}`}
              >
      ...
      <div className="p-3 border-t border-gray-800/80 text-[10px] text-gray-500 text-center">ONE CORE • ONE ARCHITECTURE</div>
    </aside>
  );
};
