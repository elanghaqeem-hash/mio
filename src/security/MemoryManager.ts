...
export class MioMemoryManager {
  ...
  public static async initialize(): Promise<void> {
    try {
      ...
      eventBus.emit('STORAGE_ERROR', { scope: 'MEMORY', action: 'INITIALIZE', error: error instanceof Error ? error.message : String(error) });
      this.initialized = true;
    }
  }
  ...
  public static setEnabled(enabled: boolean): void { this.enabled = enabled; eventBus.emit('MEMORY_UPDATED', this.getMemories()); void this.flush(); }
  public static getMemories(): MemoryItem[] { return this.memories.map((memory) => ({ ...memory })); }
  public static getPendingCandidates(): PendingMemoryCandidate[] { return this.pendingCandidates.map((candidate) => ({ ...candidate, item: { ...candidate.item } })); }
  ...
  public static proposeMemory(item: Omit<MemoryItem, 'id' | 'timestamp'>): MemoryWriteResult {
    ...
    if (decision.decision === 'REVIEW') {
      const candidate: PendingMemoryCandidate = { id: `mem_candidate_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`, createdAt: Date.now(), item: { ...item }, reason: decision.reason };
  ...
  public static addMemory(item: Omit<MemoryItem, 'id' | 'timestamp'>): MemoryItem | null { const result = this.proposeMemory(item); return result.status === 'SAVED' ? result.memory : null; }
  ...
  public static updateMemory(id: string, content: string, confidence = 1): boolean {
    ...
    memory.content = content; memory.confidence = confidence;
    eventBus.emit('MEMORY_UPDATED', this.getMemories()); void this.flush(); return true;
  }

  public static deleteMemory(id: string): boolean {
    const before = this.memories.length; this.memories = this.memories.filter((memory) => memory.id !== id); const removed = this.memories.length < before;
    if (removed) { eventBus.emit('MEMORY_UPDATED', this.getMemories()); void this.flush(); }
    return removed;
  }

  public static clearAll(): void {
    this.memories = []; this.pendingCandidates = [];
    eventBus.emit('MEMORY_UPDATED', this.getMemories()); eventBus.emit('MEMORY_REVIEW_QUEUE_UPDATED', this.getPendingCandidates()); void this.flush();
  }

  public static async flush(): Promise<void> {
    try {
      await this.storage.set<PersistedMemoryState>('memory', MEMORY_STORAGE_KEY, { enabled: this.enabled, memories: this.memories, pendingCandidates: this.pendingCandidates });
    } catch (error) {
      console.error('[MemoryManager] Failed to persist long-term memory.', error);
      eventBus.emit('STORAGE_ERROR', { scope: 'MEMORY', action: 'WRITE', error: error instanceof Error ? error.message : String(error) });
    }
  }

  private static saveTrustedMemory(item: Omit<MemoryItem, 'id' | 'timestamp'>): MemoryItem {
    const memory: MemoryItem = { ...item, id: `mem_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`, timestamp: Date.now() };
    this.memories.push(memory); eventBus.emit('MEMORY_UPDATED', this.getMemories()); void this.flush(); return memory;
  }
}
