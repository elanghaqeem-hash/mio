...
import { importSFXSample, chooseWaveformLevel, getRuntimeSample, hasRuntimeSample, relinkSFXSample, releaseRuntimeSample } from '../../creative/SFXSampleRegistry';
...
import { moveTimelineRegions, normalizeTimelineViewport, snapTimelineTime, timelineExtent, timelineGridStep, timelineTimeToX, timelineXToTime, type SFXTimelineSnapMode } from '../../creative/SFXTimelineWorkspace';
...
export const SFXStudioView: React.FC = () => {
  ...
  const [timelineSnap,setTimelineSnap]=useState<SFXTimelineSnapMode>('region');
  const [timelineSelectedIds,setTimelineSelectedIds]=useState<string[]>([]);
  const [timelinePlayhead,setTimelinePlayhead]=useState(0);
  const timelineRef=useRef<HTMLDivElement|null>(null);
  const timelineDragRef=useRef<{ids:string[];startTime:number;origin:NonNullable<MioSFXPatch['sampleRegions']>}|null>(null);
  ...
  const selectedSampleMetadataMissing=Boolean(selectedSampleRegion&&!selectedSampleAsset);const selectedSampleMissing=Boolean(selectedSampleRegion&&(selectedSampleMetadataMissing||!hasRuntimeSample(selectedSampleRegion.assetId)));
  ...
  const removeSelectedSample=()=>{if(!selectedSampleRegion)return;const assetId=selectedSampleRegion.assetId,remaining=(patch.sampleRegions??[]).filter(region=>region.id!==selectedSampleRegion.id),stillUsed=remaining.some(region=>region.assetId===assetId);if(!stillUsed)releaseRuntimeSample(assetId);setPatch(current=>({...current,sampleRegions:(current.sampleRegions??[]).filter(region=>region.id!==selectedSampleRegion.id),sampleAssets:stillUsed?current.sampleAssets:(current.sampleAssets??[]).filter(asset=>asset.id!==assetId)}));setSelectedSampleRegionId(remaining[0]?.id??null);setSamplePlayhead(0);setSampleRuntimeRevision(value=>value+1);};
  ...
  const timelineDuration=Math.max(patch.duration,timelineExtent(patch.sampleRegions??[],patch.duration));
  const timelineViewport=normalizeTimelineViewport({start:timelineScroll,end:timelineScroll+Math.max(.02,(timelineRef.current?.clientWidth??900)/timelineZoom),pixelsPerSecond:timelineZoom},timelineDuration);
  const timelineWidth=Math.max(900,timelineDuration*timelineZoom);
  const timelineStep=timelineGridStep(timelineZoom);
  const seekTimeline=(clientX:number)=>{const el=timelineRef.current;if(!el)return;const rect=el.getBoundingClientRect(),x=clientX-rect.left+el.scrollLeft,time=timelineXToTime(x,{start:0,end:timelineDuration,pixelsPerSecond:timelineZoom});setTimelinePlayhead(snapTimelineTime(time,timelineSnap,timelineViewport,patch.sampleRegions??[]));};
  const startTimelineDrag=(event:React.PointerEvent,regionId:string)=>{event.stopPropagation();const additive=event.shiftKey;const ids=additive?(timelineSelectedIds.includes(regionId)?timelineSelectedIds:[...timelineSelectedIds,regionId]):timelineSelectedIds.includes(regionId)?timelineSelectedIds:[regionId];setTimelineSelectedIds(ids);setSelectedSampleRegionId(regionId);const el=timelineRef.current;if(!el)return;const rect=el.getBoundingClientRect(),startTime=timelineXToTime(event.clientX-rect.left+el.scrollLeft,{start:0,end:timelineDuration,pixelsPerSecond:timelineZoom});timelineDragRef.current={ids,startTime,origin:(patch.sampleRegions??[]).map(region=>({...region}))};event.currentTarget.setPointerCapture(event.pointerId);};
  const moveTimelineDrag=(event:React.PointerEvent)=>{const drag=timelineDragRef.current,el=timelineRef.current;if(!drag||!el)return;const rect=el.getBoundingClientRect(),time=timelineXToTime(event.clientX-rect.left+el.scrollLeft,{start:0,end:timelineDuration,pixelsPerSecond:timelineZoom}),next=moveTimelineRegions(drag.origin,drag.ids,time-drag.startTime,timelineSnap,timelineViewport);setPatch(current=>({...current,duration:Math.max(current.duration,timelineExtent(next,current.duration)),sampleRegions:next}));};
  const endTimelineDrag=()=>{timelineDragRef.current=null;};
  ...
  return (
    <div className="relative flex h-full w-full overflow-hidden bg-[#07090e] font-mono text-xs">
      ...
      <section className="flex flex-1 flex-col overflow-y-auto bg-[#0a0e17] p-4">
        ...
        <div className="mb-4 rounded-xl border border-gray-800 bg-[#0b101c] p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2 text-[10px] text-gray-400"><div><span className="font-bold text-cyan-300">SFX TIMELINE</span><span className="ml-2">{timelineDuration.toFixed(2)}s · grid {timelineStep.toFixed(3)}s</span></div><div className="flex items-center gap-2"><select value={timelineSnap} onChange={e=>setTimelineSnap(e.target.value as SFXTimelineSnapMode)} className="rounded border border-gray-700 bg-gray-900 px-2 py-1"><option value="off">SNAP OFF</option><option value="grid">GRID</option><option value="region">REGION</option></select><label>ZOOM <input type="range" min="40" max="500" step="10" value={timelineZoom} onChange={e=>setTimelineZoom(Number(e.target.value))}/></label></div></div><div ref={timelineRef} onScroll={e=>setTimelineScroll((e.currentTarget as HTMLDivElement).scrollLeft/timelineZoom)} onPointerDown={e=>seekTimeline(e.clientX)} className="relative h-32 overflow-x-auto rounded border border-gray-800 bg-black/60"><div className="relative h-full" style={{width:timelineWidth}}>{Array.from({length:Math.ceil(timelineDuration/timelineStep)+1},(_,i)=>{const time=i*timelineStep;return <div key={i} className="pointer-events-none absolute inset-y-0 border-l border-gray-800/70" style={{left:timelineTimeToX(time,{start:0,end:timelineDuration,pixelsPerSecond:timelineZoom})}}><span className="ml-1 text-[8px] text-gray-600">{time.toFixed(timelineStep<.1?2:1)}</span></div>})}{(patch.sampleRegions??[]).map((region,index)=>{const left=timelineTimeToX(region.timelineStart,{start:0,end:timelineDuration,pixelsPerSecond:timelineZoom}),width=Math.max(8,regionTimelineDuration(region)*timelineZoom),selected=timelineSelectedIds.includes(region.id)||selectedSampleRegion?.id===region.id;return <button key={region.id} onPointerDown={e=>startTimelineDrag(e,region.id)} onPointerMove={moveTimelineDrag} onPointerUp={endTimelineDrag} onPointerCancel={endTimelineDrag} className={`absolute h-8 overflow-hidden rounded border px-2 text-left text-[9px] touch-none ${selected?'border-cyan-300 bg-cyan-900/70 text-cyan-100':'border-gray-600 bg-gray-800 text-gray-300'}`} style={{left,width,top:28+(index%2)*38}} title={`${region.name} · ${region.timelineStart.toFixed(3)}s`}><span className="truncate">{region.name}</span></button>})}<div className="pointer-events-none absolute inset-y-0 w-px bg-white" style={{left:timelineTimeToX(timelinePlayhead,{start:0,end:timelineDuration,pixelsPerSecond:timelineZoom})}}/></div></div></div>

        <div className="mb-4 rounded-xl border border-gray-800 bg-[#0b101c] p-4"><div className="mb-2 flex justify-between text-[10px] text-gray-400"><span>SAMPLE WAVEFORM</span><span>{patch.sampleRegions?.length??0} REGIONS</span></div>{sampleError&&<div className="mb-3 rounded border border-red-500/40 bg-red-500/10 px-3 py-2 text-[10px] text-red-200">{sampleError}</div>}{selectedSampleMissing&&<div className="mb-3 flex items-center justify-between rounded border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-[10px] text-amber-200"><span>{selectedSampleMetadataMissing?`MISSING ASSET METADATA · ${selectedSampleRegion?.assetId}`:`MISSING SAMPLE · ${selectedSampleAsset?.name} · relink required for playback/export`}</span>{selectedSampleAsset&&<button onClick={()=>relinkInputRef.current?.click()} className="rounded border border-amber-400/50 px-2 py-1 font-bold">RELINK</button>}</div>}{(patch.sampleRegions?.length??0)>1&&<div className="mb-2 flex gap-1 overflow-x-auto">{patch.sampleRegions?.map(region=><button key={region.id} onClick={()=>{setSelectedSampleRegionId(region.id);setSamplePlayhead(0);setSampleError(null);}} className={`shrink-0 rounded border px-2 py-1 text-[9px] ${selectedSampleRegion?.id===region.id?'border-cyan-400 text-cyan-200':'border-gray-700 text-gray-500'}`}>{region.name}</button>)}</div>}<div className="overflow-x-auto"><div className="relative h-28" style={{width:`${waveformZoom*100}%`}}><canvas ref={waveformRef} width={900} height={140} onPointerDown={e=>seekWaveform(e.clientX)} className="h-28 w-full touch-none rounded bg-black cursor-crosshair"/><div className="pointer-events-none absolute top-0 h-28 w-px bg-white" style={{left:`${samplePlayhead*100}%`}}/></div></div>{selectedSampleAsset&&<div className="mt-2 text-[10px] text-gray-500">{selectedSampleAsset.name} · {selectedSampleAsset.sampleRate} Hz · {selectedSampleAsset.channels} ch</div>}{selectedSampleRegion&&<div className="mt-3 grid grid-cols-2 gap-2 text-[10px]"><button onClick={splitAtPlayhead} className="rounded border border-gray-700 py-1">SPLIT @ PLAYHEAD</button><button onClick={duplicateSelectedSample} className="rounded border border-gray-700 py-1">DUPLICATE</button><button onClick={()=>updateSampleRegion({reverse:!selectedSampleRegion.reverse})} className="rounded border border-gray-700 py-1">{selectedSampleRegion.reverse?'REVERSE ON':'REVERSE'}</button><button onClick={normalizeSelectedSample} className="rounded border border-gray-700 py-1">NORMALIZE</button><button onClick={removeSelectedSample} className="rounded border border-rose-500/30 py-1 text-rose-300">REMOVE REGION</button><button onClick={()=>updateSampleRegion({loop:!selectedSampleRegion.loop})} className="rounded border border-gray-700 py-1">{selectedSampleRegion.loop?'LOOP ON':'LOOP'}</button><label>ZOOM {waveformZoom.toFixed(1)}x<input className="w-full" type="range" min="1" max="8" step=".5" value={waveformZoom} onChange={e=>setWaveformZoom(Number(e.target.value))}/></label>{selectedSampleAsset&&<><label>TRIM START {selectedSampleRegion.sourceStart.toFixed(3)}s<input className="w-full" type="range" min="0" max={Math.max(0,selectedSampleRegion.sourceEnd-.001)} step=".001" value={selectedSampleRegion.sourceStart} onChange={e=>updateSampleRegion({sourceStart:Number(e.target.value)})}/></label><label>TRIM END {selectedSampleRegion.sourceEnd.toFixed(3)}s<input className="w-full" type="range" min={Math.min(selectedSampleDuration,selectedSampleRegion.sourceStart+.001)} max={selectedSampleDuration} step=".001" value={selectedSampleRegion.sourceEnd} onChange={e=>updateSampleRegion({sourceEnd:Number(e.target.value)})}/></label></>}<label>FADE IN {selectedSampleRegion.fadeIn.toFixed(2)}s<input className="w-full" type="range" min="0" max={Math.max(.01,(selectedSampleRegion.sourceEnd-selectedSampleRegion.sourceStart)/2)} step=".01" value={selectedSampleRegion.fadeIn} onChange={e=>updateSampleRegion({fadeIn:Number(e.target.value)})}/></label><label>FADE OUT {selectedSampleRegion.fadeOut.toFixed(2)}s<input className="w-full" type="range" min="0" max={Math.max(.01,(selectedSampleRegion.sourceEnd-selectedSampleRegion.sourceStart)/2)} step=".01" value={selectedSampleRegion.fadeOut} onChange={e=>updateSampleRegion({fadeOut:Number(e.target.value)})}/></label><label>PITCH {selectedSampleRegion.pitchSemitones}<input className="w-full" type="range" min="-24" max="24" step="1" value={selectedSampleRegion.pitchSemitones} onChange={e=>updateSampleRegion({pitchSemitones:Number(e.target.value)})}/></label><label>GAIN {selectedSampleRegion.gain.toFixed(2)}<input className="w-full" type="range" min="0" max="4" step=".01" value={selectedSampleRegion.gain} onChange={e=>updateSampleRegion({gain:Number(e.target.value)})}/></label><label>PAN {selectedSampleRegion.pan.toFixed(2)}<input className="w-full" type="range" min="-1" max="1" step=".01" value={selectedSampleRegion.pan} onChange={e=>updateSampleRegion({pan:Number(e.target.value)})}/></label></div>}</div>
