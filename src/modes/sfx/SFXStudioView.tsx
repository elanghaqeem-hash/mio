...
export const SFXStudioView: React.FC = () => {
  ...
  const relinkSelectedSample=async(file:File)=>{const context=audioCtxRef.current;if(!context||!selectedSampleAsset)return;if(context.state==='suspended')await context.resume();await relinkSFXSample(context,file,selectedSampleAsset);setPatch(current=>({...current}));};
