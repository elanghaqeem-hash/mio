...
export const GraphicStudioView: React.FC = () => {
  ...
  const [nodeEditMode,setNodeEditMode]=useState(false); const [selectedPathPointId,setSelectedPathPointId]=useState<string>(''); const [activeBezierHandle,setActiveBezierHandle]=useState<'in'|'out'|null>(null);
  ...
  const onCanvasPointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if(nodeEditMode&&selectedLayer?.type==='vector'&&selectedLayer.path&&!selectedLayer.locked){const local=graphicLayerWorldToLocal(selectedLayer,point),selected=selectedLayer.path.points.find(p=>p.id===selectedPathPointId);if(selected){const handleHit=(['in','out'] as const).find(kind=>{const hx=kind==='in'?selected.inX:selected.outX,hy=kind==='in'?selected.inY:selected.outY;return hx!==undefined&&hy!==undefined&&Math.hypot(local.x-hx,local.y-hy)<=9;});if(handleHit){setActiveBezierHandle(handleHit);event.currentTarget.setPointerCapture(event.pointerId);return;}}const hit=selectedLayer.path.points.find(p=>Math.hypot(local.x-p.x,local.y-p.y)<=9);if(hit){setSelectedPathPointId(hit.id);setActiveBezierHandle(null);event.currentTarget.setPointerCapture(event.pointerId);return;}}
  ...
  const onCanvasPointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
    const point=canvasPoint(event);
    if(nodeEditMode&&selectedLayer?.type==='vector'&&selectedPathPointId&&event.currentTarget.hasPointerCapture(event.pointerId)){const local=graphicLayerWorldToLocal(selectedLayer,point);if(activeBezierHandle){const anchor=selectedLayer.path?.points.find(p=>p.id===selectedPathPointId);if(anchor)setDocumentData(previous=>setGraphicPathPointHandles(previous,selectedLayer.id,selectedPathPointId,activeBezierHandle==='in'?local:(anchor.inX!==undefined&&anchor.inY!==undefined?{x:anchor.inX,y:anchor.inY}:undefined),activeBezierHandle==='out'?local:(anchor.outX!==undefined&&anchor.outY!==undefined?{x:anchor.outX,y:anchor.outY}:undefined)));return;}setDocumentData(previous=>moveGraphicPathPoint(previous,selectedLayer.id,selectedPathPointId,local));return;}
  ...
  const onCanvasPointerUp = (event: React.PointerEvent<HTMLCanvasElement>) => { dragSessionRef.current=null; resizeSessionRef.current=null; rotationSessionRef.current=null; groupResizeSessionRef.current=null; activeBezierHandle&&setActiveBezierHandle(null); setSmartGuides([]); if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId); };
  ...
  useEffect(() => {
    ...
    for (const layer of documentData.layers) {
      ...
      if(nodeEditMode&&selectedLayerId===layer.id&&layer.type==='vector'&&layer.path){ctx.setLineDash([]);for(const point of layer.path.points){const world=graphicLayerLocalToWorld(layer,point),px=world.x,py=world.y;if(point.id===selectedPathPointId){for(const kind of ['in','out'] as const){const hx=kind==='in'?point.inX:point.outX,hy=kind==='in'?point.inY:point.outY;if(hx!==undefined&&hy!==undefined){const hw=graphicLayerLocalToWorld(layer,{x:hx,y:hy});ctx.strokeStyle='#f472b6';ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(hw.x,hw.y);ctx.stroke();ctx.beginPath();ctx.arc(hw.x,hw.y,5,0,Math.PI*2);ctx.fillStyle='#f472b6';ctx.fill();}}}ctx.fillStyle=point.id===selectedPathPointId?'#f472b6':'#e2e8f0';ctx.strokeStyle='#0284c7';ctx.fillRect(px-4,py-4,8,8);ctx.strokeRect(px-4,py-4,8,8);}}
