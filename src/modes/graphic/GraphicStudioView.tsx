...
export const GraphicStudioView: React.FC = () => {
  ...
  const [hoveredSegment,setHoveredSegment]=useState<{layerId:string;segmentIndex:number;t:number;point:{x:number;y:number}}|null>(null); const [penMode,setPenMode]=useState(false); const [penDragPointId,setPenDragPointId]=useState(''); const [penDragOrigin,setPenDragOrigin]=useState<{x:number;y:number}|null>(null); const [penPreviewPoint,setPenPreviewPoint]=useState<{x:number;y:number}|null>(null); const [activePenLayerId,setActivePenLayerId]=useState(''); const [nodeEditMode,setNodeEditMode]=useState(false); const [selectedPathPointId,setSelectedPathPointId]=useState<string>(''); const [activeBezierHandle,setActiveBezierHandle]=useState<'in'|'out'|null>(null);
  ...
  const clearPenSession=()=>{setActivePenLayerId('');setPenDragPointId('');setPenDragOrigin(null);setPenPreviewPoint(null);};
  ...
  const onCanvasPointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => { const penPoint=canvasPoint(event); if(penMode){if(!activePenLayerId){const started=beginGraphicPenPath(documentData,penPoint);setDocumentData(started.document);setActivePenLayerId(started.layerId);setSelectedLayerIds([started.layerId]);setPenDragPointId(started.pointId);setPenDragOrigin(penPoint);event.currentTarget.setPointerCapture(event.pointerId);}else{const active=documentData.layers.find(layer=>layer.id===activePenLayerId);const first=active?.path?.points[0];if(first&&active.path!.points.length>=3&&Math.hypot(penPoint.x-(active.x+first.x),penPoint.y-(active.y+first.y))<=10){setDocumentData(previous=>finishGraphicPenPath(previous,activePenLayerId,true));setActivePenLayerId('');}else{const local=active?graphicLayerWorldToLocal(active,penPoint):penPoint;const appended=appendGraphicPenPoint(documentData,activePenLayerId,local);setDocumentData(appended.document);if(appended.pointId){setPenDragPointId(appended.pointId);setPenDragOrigin(penPoint);event.currentTarget.setPointerCapture(event.pointerId);}}}return;}
    const point=canvasPoint(event);
    if(nodeEditMode&&selectedLayer?.type==='vector'&&selectedLayer.path&&!selectedLayer.locked){const local=graphicLayerWorldToLocal(selectedLayer,point),nearest=nearestGraphicPathSegmentPoint(selectedLayer.path,local,36);if(nearest&&nearest.distance<=8&&!selectedLayer.path.points.some(p=>Math.hypot(local.x-p.x,local.y-p.y)<=9)){const next=insertGraphicPathPointOnSegment(documentData,selectedLayer.id,nearest.segment.index,nearest.t);const nextLayer=next.layers.find(l=>l.id===selectedLayer.id),inserted=nextLayer?.path?.points.find(p=>!selectedLayer.path!.points.some(old=>old.id===p.id));setDocumentData(next);if(inserted)setSelectedPathPointId(inserted.id);setHoveredSegment(null);return;}}
  ...
  const onCanvasPointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => { if(penMode&&activePenLayerId)setPenPreviewPoint(canvasPoint(event)); if(penMode&&activePenLayerId&&penDragPointId&&penDragOrigin){const world=canvasPoint(event);if(Math.hypot(world.x-penDragOrigin.x,world.y-penDragOrigin.y)>=3){const active=documentData.layers.find(layer=>layer.id===activePenLayerId);if(active){const local=graphicLayerWorldToLocal(active,world),anchor=active.path?.points.find(point=>point.id===penDragPointId);if(anchor)setDocumentData(previous=>setGraphicSmoothPathHandle(setGraphicPathNodeType(previous,activePenLayerId,penDragPointId,'smooth'),activePenLayerId,penDragPointId,'out',local));}}return;}
    const point=canvasPoint(event);
    if(nodeEditMode&&selectedLayer?.type==='vector'&&selectedLayer.path&&!event.currentTarget.hasPointerCapture(event.pointerId)){const local=graphicLayerWorldToLocal(selectedLayer,point),nearest=nearestGraphicPathSegmentPoint(selectedLayer.path,local,36);setHoveredSegment(nearest&&nearest.distance<=8?{layerId:selectedLayer.id,segmentIndex:nearest.segment.index,t:nearest.t,point:graphicLayerLocalToWorld(selectedLayer,nearest.point)}:null);}else if(hoveredSegment)setHoveredSegment(null);
  ...
  useEffect(() => {
    ...
    if(nodeEditMode&&hoveredSegment){ctx.save();ctx.fillStyle='#f472b6';ctx.strokeStyle='#f472b6';ctx.beginPath();ctx.arc(hoveredSegment.point.x,hoveredSegment.point.y,5,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.restore();}
  ...
  }, [documentData, selectedLayerIds, smartGuides, penMode, activePenLayerId, penPreviewPoint, nodeEditMode, hoveredSegment, selectedPathPointId]);
  ...
  return (
    <div className="relative flex h-full w-full bg-[#07090e] font-mono text-xs overflow-hidden">
      <CreativeWorkspaceToolbar workspace={workspace} /><button onClick={()=>{setPenMode(value=>!value);clearPenSession();setNodeEditMode(false);}} className={`rounded border px-2 py-1 text-xs ${penMode?'border-cyan-400 text-cyan-300':'border-gray-700 text-gray-400'}`}>PEN</button>
