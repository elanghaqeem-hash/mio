...
export const TaskMonitorView: React.FC = () => {
  ...
  const [selectedTaskId, setSelectedTaskId] = useState<string | null>(() => taskRuntime.snapshot().tasks[0]?.id ?? null);

  useEffect(() => {
    const update = (next: TaskRuntimeSnapshot) => setSnapshot(next);
    const unsub = eventBus.on<TaskRuntimeSnapshot>('TASK_RUNTIME_SNAPSHOT', update);
    setSnapshot(taskRuntime.snapshot());
    return unsub;
  }, []);
    ...
    <div className="h-full w-full bg-[#07090e] font-mono text-xs overflow-hidden flex flex-col">
      <div className="p-4 border-b border-gray-800 bg-[#0d121d] flex items-center justify-between">
        <div>
          ...
          <div className="text-[10px] text-gray-500 mt-1">Live lifecycle, progress, permission waits, retries, cancellation, and STOP MIO propagation.</div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => taskRuntime.clearCompleted()}
            className="flex items-center gap-1 px-2 py-1 rounded border border-gray-700 text-gray-400 hover:text-white hover:border-gray-500"
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-[42%] border-r border-gray-800 overflow-y-auto p-3 space-y-2">
          {snapshot.tasks.length === 0 && (
            <div className="h-full flex items-center justify-center text-center text-gray-600 px-8">No runtime tasks yet. Send a directive in Chat, Research, or Project mode to create one.</div>
          )}
          {snapshot.tasks.map((task) => (
            <button
              key={task.id}
              onClick={() => setSelectedTaskId(task.id)}
              className={`w-full text-left p-3 rounded-xl border transition ${selectedTaskId === task.id ? 'border-cyan-500/50 bg-cyan-950/20' : 'border-gray-800 bg-[#0d121d] hover:border-gray-700'}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-gray-200 font-bold truncate">{task.title}</div>
                  <div className="text-[10px] text-gray-500 mt-1">{task.mode} • {task.id}</div>
                </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {!selectedTask ? (
            <div className="h-full flex items-center justify-center text-gray-600">Select a task to inspect runtime details.</div>
          ) : (
            <div className="space-y-4 max-w-4xl">
              <div className="p-4 rounded-xl border border-gray-800 bg-[#0d121d]">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-base font-bold text-white">{selectedTask.title}</div>
                    <div className="text-[10px] text-gray-500 mt-1">Project: {selectedTask.projectId ?? 'workspace'} • Mode: {selectedTask.mode}</div>
                  </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {selectedTask.status === 'RUNNING' && (
                    <button onClick={() => taskRuntime.pause(selectedTask.id)} className="flex items-center gap-1 px-3 py-1.5 rounded border border-violet-500/40 text-violet-300 hover:bg-violet-950/30"><Pause size={13} /> Pause</button>
                  )}
                  {selectedTask.status === 'PAUSED' && (
                    <button onClick={() => taskRuntime.resume(selectedTask.id)} className="flex items-center gap-1 px-3 py-1.5 rounded border border-cyan-500/40 text-cyan-300 hover:bg-cyan-950/30"><Play size={13} /> Resume</button>
                  )}
                  {selectedTask.status === 'FAILED' && selectedTask.retryCount < selectedTask.maxRetries && (
                    <button onClick={() => taskRuntime.scheduleRetry(selectedTask.id)} className="flex items-center gap-1 px-3 py-1.5 rounded border border-amber-500/40 text-amber-300 hover:bg-amber-950/30"><RotateCcw size={13} /> Retry</button>
                  )}
                  {!['COMPLETED', 'FAILED', 'CANCELLED'].includes(selectedTask.status) && (
                    <button onClick={() => taskRuntime.cancel(selectedTask.id, 'Cancelled from Task Monitor')} className="flex items-center gap-1 px-3 py-1.5 rounded border border-red-500/40 text-red-300 hover:bg-red-950/30"><Square size={13} /> Cancel</button>
                  )}
                  <button
                    onClick={() => emergencyStop.triggerEmergencyStop('STOP MIO from Task Monitor')}
                    className="flex items-center gap-1 px-3 py-1.5 rounded bg-red-700 hover:bg-red-600 text-white font-bold"
                  >
                    <Square size={13} /> STOP MIO
                  </button>
                </div>
              </div>
                  ...
                  {selectedTask.steps.map((step, index) => (
                    <div key={step.id} className="flex items-start gap-3 p-3 rounded-lg border border-gray-800 bg-[#111726]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[9px] text-gray-500 mt-1">{step.mode}{step.requiresPermission ? ' • permission-gated' : ''}</div>
                        {step.error && <div className="text-[10px] text-red-300 mt-1">{step.error}</div>}
                      </div>
