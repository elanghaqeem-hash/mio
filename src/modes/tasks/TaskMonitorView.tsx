        ...
        <div className="w-[42%] border-r border-gray-800 overflow-y-auto p-3 space-y-2">
          ...
          {snapshot.tasks.map((task) => (
            <button
              ...
              className={`w-full text-left p-3 rounded-xl border transition ${selectedTask?.id === task.id ? 'border-cyan-500/50 bg-cyan-950/20' : 'border-gray-800 bg-[#0d121d] hover:border-gray-700'}`}
            >
            ...
            <div className="space-y-4 max-w-4xl">
              <div className="p-4 rounded-xl border border-gray-800 bg-[#0d121d]">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {selectedTask.status === 'RUNNING' && <button onClick={() => taskRuntime.pause(selectedTask.id)} className="flex items-center gap-1 px-3 py-1.5 rounded border border-violet-500/40 text-violet-300 hover:bg-violet-950/30"><Pause size={13} /> Pause</button>}
                  ...
                  {selectedTask.status === 'FAILED' && selectedTask.retryCount < selectedTask.maxRetries && <button onClick={() => taskRuntime.scheduleRetry(selectedTask.id)} className="flex items-center gap-1 px-3 py-1.5 rounded border border-amber-500/40 text-amber-300 hover:bg-amber-950/30"><RotateCcw size={13} /> Retry</button>}
                  {!['COMPLETED', 'FAILED', 'CANCELLED'].includes(selectedTask.status) && <button onClick={() => taskRuntime.cancel(selectedTask.id, 'Cancelled from Task Monitor')} className="flex items-center gap-1 px-3 py-1.5 rounded border border-red-500/40 text-red-300 hover:bg-red-950/30"><Square size={13} /> Cancel</button>}
