  ...
  return (
    <div className="flex h-full w-full flex-col space-y-6 overflow-y-auto bg-[#07090e] p-4 font-mono text-xs">
      ...
      <div className="rounded-xl border border-gray-800 bg-[#0d121d] p-4">
        ...
        {activeGrants.length === 0 ? <div className="p-3 text-center text-gray-500">No active authorization grants. MIO has no reusable execution authority.</div> : <div className="space-y-2">{activeGrants.map((grant) => <div key={grant.id} className="rounded-lg border border-gray-800 bg-[#111726] p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded border border-cyan-500/30 bg-cyan-950 px-2 py-0.5 text-[9px] font-bold text-cyan-300">{grant.level}</span><span className="truncate font-bold text-white">{grant.scope.action}</span></div><div className="mt-1 text-[10px] text-gray-400">{grant.scope.target}</div><div className="mt-2 flex flex-wrap gap-2 text-[9px] text-gray-500"><span>task: {grant.scope.taskId}</span>{grant.scope.projectId && <span>project: {grant.scope.projectId}</span>}{grant.scope.resourceId && <span>resource: {grant.scope.resourceId}</span>}</div></div><button onClick={() => PermissionEngine.revokeGrant(grant.id, 'User revoked grant from Security Control Center')} className="shrink-0 cursor-pointer rounded border border-red-500/30 px-2 py-1 text-[9px] text-red-300 hover:bg-red-950/30">REVOKE</button></div><div className="mt-2 flex items-center gap-3 border-t border-gray-800 pt-2 text-[9px] text-gray-500"><span className="flex items-center gap-1"><Clock3 size={10} /> expires {new Date(grant.expiresAt).toLocaleTimeString()}</span><span>uses {grant.uses}/{grant.maxUses}</span><span>source {grant.source}</span></div></div>)}</div>}
      </div>
