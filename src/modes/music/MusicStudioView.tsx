...
export const MusicStudioView: React.FC = () => {
  ...
  const connectTrackEffects = (ctx: BaseAudioContext, input: AudioNode, track: MusicTrack, destination: AudioNode, step:number, exposeParams=false) => { let node=input; for(const effect of track.effects??[]){ if(!effect.enabled)continue; const amount=automationValue(track.id,'effectAmount',step,effect.id)??effect.amount, mix=automationValue(track.id,'effectMix',step,effect.id)??effect.mix??0.35, feedbackValue=automationValue(track.id,'effectFeedback',step,effect.id)??effect.feedback??0.25, resonance=automationValue(track.id,'effectResonance',step,effect.id)??effect.resonance??0; if(effect.type==='gain'){const gain=ctx.createGain();gain.gain.value=mapMusicAutomationValue('effectAmount',amount,effect.type);node.connect(gain);node=gain;if(exposeParams)liveFxParamRef.current.set(effect.id,{amount:gain.gain});} else if(effect.type==='lowpass'){const filter=ctx.createBiquadFilter();filter.type='lowpass';filter.frequency.value=mapMusicAutomationValue('effectAmount',amount,effect.type);filter.Q.value=mapMusicAutomationValue('effectResonance',resonance,effect.type);node.connect(filter);node=filter;if(exposeParams)liveFxParamRef.current.set(effect.id,{amount:filter.frequency,resonance:filter.Q});} else if(effect.type==='delay'){const dry=ctx.createGain(),wet=ctx.createGain(),delay=ctx.createDelay(1),feedback=ctx.createGain(),sum=ctx.createGain();dry.gain.value=1-mix;wet.gain.value=mix;delay.delayTime.value=mapMusicAutomationValue('effectAmount',amount,effect.type);feedback.gain.value=mapMusicAutomationValue('effectFeedback',feedbackValue,effect.type);node.connect(dry);dry.connect(sum);node.connect(delay);delay.connect(wet);wet.connect(sum);delay.connect(feedback);feedback.connect(delay);node=sum;if(exposeParams)liveFxParamRef.current.set(effect.id,{amount:delay.delayTime,mix:wet.gain,feedback:feedback.gain});} } node.connect(destination); };
