...
export const MusicStudioView: React.FC = () => {
  ...
  const scheduleLiveAutomation=(track:MusicTrack,step:number,now:number)=>{const channel=trackChannelRef.current.get(track.id);if(!channel)return;const schedule=(parameter:string,targetId:string|undefined,param:AudioParam,map:(value:number)=>number)=>{const lane=project.automationLanes?.find(candidate=>candidate.trackId===track.id&&candidate.parameter===parameter&&candidate.targetId===targetId);if(!lane)return;const value=evaluateMusicAutomationLane(lane,step);if(value===undefined)return;const mapped=map(value),duration=60/project.tempo/4;const mode=lane.interpolation??'linear';if(mode==='step')param.setValueAtTime(mapped,now);else if(mode==='linear')param.linearRampToValueAtTime(mapped,now+duration);else{const current=param.value;param.setValueCurveAtTime(new Float32Array([current,current+(mapped-current)*0.15625,current+(mapped-current)*0.5,current+(mapped-current)*0.84375,mapped]),now,duration);}};schedule('volume',undefined,channel.volume.gain,value=>value);schedule('pan',undefined,channel.panner.pan,value=>value*2-1);for(const [busId,sendNode] of channel.sends)schedule('sendLevel',busId,sendNode.gain,value=>value);for(const effect of track.effects??[]){const handles=liveFxParamRef.current.get(effect.id);if(!handles)continue;if(handles.amount)schedule('effectAmount',effect.id,handles.amount,value=>effect.type==='gain'?0.5+value:effect.type==='lowpass'?200+value*19800:value*0.5);if(handles.mix)schedule('effectMix',effect.id,handles.mix,value=>value);if(handles.feedback)schedule('effectFeedback',effect.id,handles.feedback,value=>Math.min(0.9,value));if(handles.resonance)schedule('effectResonance',effect.id,handles.resonance,value=>value*20);}};
