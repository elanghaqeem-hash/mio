...
export const MusicStudioView: React.FC = () => {
  ...
  const ensureLiveTrackChannel=(ctx:AudioContext,track:MusicTrack,step:number)=>{const existing=trackChannelRef.current.get(track.id);if(existing)return existing;const input=ctx.createGain(),panner=ctx.createStereoPanner(),output=ctx.createGain();input.connect(panner);connectTrackEffects(ctx,panner,track,output,step);output.connect(ctx.destination);for(const send of track.sends??[]){const bus=project.returnBuses?.find(candidate=>candidate.id===send.busId),level=automationValue(track.id,'sendLevel',step,send.busId)??send.level;if(!send.enabled||!bus||level<=0)continue;const sendGain=ctx.createGain();sendGain.gain.value=level;panner.connect(sendGain);sendGain.connect(ensureLiveReturnChannel(ctx,bus,track,step).input);}trackChannelRef.current.set(track.id,{input,panner,output});return {input,panner,output};};
  const clearLiveTrackChannels=()=>{for(const channel of trackChannelRef.current.values()){try{channel.input.disconnect();channel.panner.disconnect();channel.output.disconnect();}catch{/* already disconnected */}}trackChannelRef.current.clear();for(const channel of returnChannelRef.current.values()){try{channel.input.disconnect();channel.output.disconnect();}catch{/* already disconnected */}}returnChannelRef.current.clear();};
