  ...
  return (
    <div className="relative flex h-full w-full bg-[#07090e] font-mono text-xs overflow-hidden">
      ...
      <div className="flex-1 flex flex-col p-4 bg-[#0a0e17] overflow-hidden">
        ...
        <div className="mb-3 rounded-xl border border-gray-800 bg-[#090d16] p-2"><div className="mb-2 flex items-center justify-between"><span className="font-bold text-violet-300">TRACK AUTOMATION</span><select value={automationParameter} onChange={event=>setAutomationParameter(event.target.value as 'volume'|'pan')} className="bg-gray-900 text-[10px] text-violet-200"><option value="volume">VOLUME</option><option value="pan">PAN</option></select></div><div ref={automationEditorRef} onPointerDown={event=>{if(!activeTrack)return;const rect=event.currentTarget.getBoundingClientRect(),total=project.arrangement?.totalSteps??project.totalSteps,step=Math.max(0,Math.min(total-1,Math.round(((event.clientX-rect.left)/Math.max(1,rect.width))*(total-1)))),value=Math.max(0,Math.min(1,1-(event.clientY-rect.top)/Math.max(1,rect.height))),laneId=`automation_${activeTrack.id}_${automationParameter}`;setProject(current=>{const lanes=current.automationLanes??[],existing=lanes.find(lane=>lane.id===laneId),point={id:`music_auto_${Date.now()}`,step,value};return {...current,automationLanes:existing?lanes.map(lane=>lane.id===laneId?{...lane,points:[...lane.points.filter(item=>item.step!==step),point].sort((a,b)=>a.step-b.step)}:lane):[...lanes,{id:laneId,trackId:activeTrack.id,parameter:automationParameter,interpolation:'linear',points:[point]}]};});}} className="relative h-24 touch-none rounded border border-gray-800 bg-gray-950">{(()=>{const lane=project.automationLanes?.find(item=>item.trackId===activeTrackId&&item.parameter===automationParameter&&!item.targetId),total=project.arrangement?.totalSteps??project.totalSteps;return lane?.points.map(point=><span key={point.id??point.step} className="absolute h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border border-violet-200 bg-violet-500" style={{left:`${(point.step/Math.max(1,total-1))*100}%`,top:`${(1-point.value)*100}%`}} />);})()}</div><div className="mt-1 text-[9px] text-gray-500">Tap or drag-ready surface · 0–100% normalized automation</div></div>
