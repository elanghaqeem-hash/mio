...
export const MusicStudioView: React.FC = () => {
  ...
  const trackChannelRef=useRef<Map<string,{input:GainNode;panner:StereoPannerNode;output:GainNode;sends:Map<string,GainNode>}>>(new Map());
  ...
  const scheduleLiveAutomation=(track:MusicTrack,step:number,now:number)=>{const channel=trackChannelRef.current.get(track.id);if(!channel)return;const pan=automationValue(track.id,'pan',step);if(pan!==undefined)channel.panner.pan.setValueAtTime(pan*2-1,now);for(const [busId,sendNode] of channel.sends){const value=automationValue(track.id,'sendLevel',step,busId);if(value!==undefined)sendNode.gain.setValueAtTime(value,now);}for(const effect of track.effects??[]){const handles=liveFxParamRef.current.get(effect.id);if(!handles)continue;const amount=automationValue(track.id,'effectAmount',step,effect.id),mix=automationValue(track.id,'effectMix',step,effect.id),feedback=automationValue(track.id,'effectFeedback',step,effect.id),resonance=automationValue(track.id,'effectResonance',step,effect.id);if(amount!==undefined&&handles.amount)handles.amount.setValueAtTime(effect.type==='gain'?0.5+amount:effect.type==='lowpass'?200+amount*19800:amount*0.5,now);if(mix!==undefined&&handles.mix)handles.mix.setValueAtTime(mix,now);if(feedback!==undefined&&handles.feedback)handles.feedback.setValueAtTime(Math.min(0.9,feedback),now);if(resonance!==undefined&&handles.resonance)handles.resonance.setValueAtTime(resonance*20,now);}};
  ...
  const ensureLiveTrackChannel=(ctx:AudioContext,track:MusicTrack,step:number)=>{const existing=trackChannelRef.current.get(track.id);if(existing)return existing;const input=ctx.createGain(),panner=ctx.createStereoPanner(),output=ctx.createGain(),sends=new Map<string,GainNode>();input.connect(panner);connectTrackEffects(ctx,panner,track,output,step,true);output.connect(ctx.destination);for(const send of track.sends??[]){const bus=project.returnBuses?.find(candidate=>candidate.id===send.busId),level=automationValue(track.id,'sendLevel',step,send.busId)??send.level;if(!send.enabled||!bus)continue;const sendGain=ctx.createGain();sendGain.gain.value=Math.max(0,level);panner.connect(sendGain);sendGain.connect(ensureLiveReturnChannel(ctx,bus,track,step).input);sends.set(send.busId,sendGain);}const channel={input,panner,output,sends};trackChannelRef.current.set(track.id,channel);return channel;};
  const clearLiveTrackChannels=()=>{for(const channel of trackChannelRef.current.values()){try{channel.input.disconnect();channel.panner.disconnect();channel.output.disconnect();for(const send of channel.sends.values())send.disconnect();}catch{/* already disconnected */}}trackChannelRef.current.clear();for(const channel of returnChannelRef.current.values()){try{channel.input.disconnect();channel.output.disconnect();}catch{/* already disconnected */}}returnChannelRef.current.clear();liveFxParamRef.current.clear();};
