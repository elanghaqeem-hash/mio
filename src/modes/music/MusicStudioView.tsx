...
export const MusicStudioView: React.FC = () => {
  ...
  const [automationParameter,setAutomationParameter]=useState<'volume'|'pan'>('volume');
  ...
  return (
    <div className="relative flex h-full w-full bg-[#07090e] font-mono text-xs overflow-hidden">
      ...
      <div className="flex-1 flex flex-col p-4 bg-[#0a0e17] overflow-hidden">
        ...
        <div className="mb-3 rounded-xl border border-gray-800 bg-[#090d16] p-2"><div className="mb-2 flex items-center justify-between"><span className="font-bold text-violet-300">TRACK AUTOMATION</span><div className="flex gap-1"><button disabled={!selectedAutomationPointId} onClick={()=>{if(!selectedAutomationPointId)return;setProject(current=>({...current,automationLanes:(current.automationLanes??[]).map(lane=>lane.trackId===activeTrackId&&lane.parameter===automationParameter&&!lane.targetId?{...lane,points:lane.points.filter(point=>point.id!==selectedAutomationPointId)}:lane)}));setSelectedAutomationPointId(null);}} className="text-[9px] text-red-300 disabled:text-gray-700">DELETE</button><button onClick={()=>setProject(current=>({...current,automationLanes:(current.automationLanes??[]).map(lane=>lane.trackId===activeTrackId&&lane.parameter===automationParameter&&!lane.targetId?{...lane,interpolation:lane.interpolation==='linear'?'step':lane.interpolation==='step'?'smooth':'linear'}:lane)}))} className="text-[9px] text-violet-300">{project.automationLanes?.find(lane=>lane.trackId===activeTrackId&&lane.parameter===automationParameter&&!lane.targetId)?.interpolation?.toUpperCase()??'LINEAR'}</button><select value={automationParameter} onChange={event=>setAutomationParameter(event.target.value as 'volume'|'pan')} className="bg-gray-900 text-[10px] text-violet-200"><option value="volume">VOLUME</option><option value="pan">PAN</option></select></div></div><div ref={automationEditorRef} onPointerDown={event=>{if(!activeTrack)return;const rect=event.currentTarget.getBoundingClientRect(),total=project.arrangement?.totalSteps??project.totalSteps,step=Math.max(0,Math.min(total-1,Math.round(((event.clientX-rect.left)/Math.max(1,rect.width))*(total-1)))),value=Math.max(0,Math.min(1,1-(event.clientY-rect.top)/Math.max(1,rect.height))),laneId=`automation_${activeTrack.id}_${automationParameter}`;setProject(current=>{const lanes=current.automationLanes??[],existing=lanes.find(lane=>lane.id===laneId),point={id:`music_auto_${Date.now()}`,step,value};return {...current,automationLanes:existing?lanes.map(lane=>lane.id===laneId?{...lane,points:[...lane.points.filter(item=>item.step!==step),point].sort((a,b)=>a.step-b.step)}:lane):[...lanes,{id:laneId,trackId:activeTrack.id,parameter:automationParameter,interpolation:'linear',points:[point]}]};});}} className="relative h-24 touch-none rounded border border-gray-800 bg-gray-950">{(()=>{const lane=project.automationLanes?.find(item=>item.trackId===activeTrackId&&item.parameter===automationParameter&&!item.targetId),total=project.arrangement?.totalSteps??project.totalSteps;return lane?.points.map(point=><button key={point.id??point.step} aria-label={`Automation point step ${point.step}`} onPointerDown={event=>{event.stopPropagation();setSelectedAutomationPointId(point.id??null);}} className={`absolute h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border ${selectedAutomationPointId===point.id?'border-white bg-violet-300':'border-violet-200 bg-violet-500'}`} style={{left:`${(point.step/Math.max(1,total-1))*100}%`,top:`${(1-point.value)*100}%`}} />);})()}</div><div className="mt-1 text-[9px] text-gray-500">Tap or drag-ready surface · 0–100% normalized automation</div></div>
