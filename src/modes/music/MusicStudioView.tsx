...
export const MusicStudioView: React.FC = () => {
  ...
  const connectTrackEffects = (ctx: BaseAudioContext, input: AudioNode, track: MusicTrack, destination: AudioNode, step:number, paramRegistry?:Map<string,{amount?:AudioParam;mix?:AudioParam;feedback?:AudioParam;resonance?:AudioParam}>) => { let node=input; for(const effect of track.effects??[]){ if(!effect.enabled)continue; const amount=automationValue(track.id,'effectAmount',step,effect.id)??effect.amount, mix=automationValue(track.id,'effectMix',step,effect.id)??effect.mix??0.35, feedbackValue=automationValue(track.id,'effectFeedback',step,effect.id)??effect.feedback??0.25, resonance=automationValue(track.id,'effectResonance',step,effect.id)??effect.resonance??0; if(effect.type==='gain'){const gain=ctx.createGain();gain.gain.value=mapMusicAutomationValue('effectAmount',amount,effect.type);node.connect(gain);node=gain;paramRegistry?.set(effect.id,{amount:gain.gain});} else if(effect.type==='lowpass'){const filter=ctx.createBiquadFilter();filter.type='lowpass';filter.frequency.value=mapMusicAutomationValue('effectAmount',amount,effect.type);filter.Q.value=mapMusicAutomationValue('effectResonance',resonance,effect.type);node.connect(filter);node=filter;paramRegistry?.set(effect.id,{amount:filter.frequency,resonance:filter.Q});} else if(effect.type==='delay'){const dry=ctx.createGain(),wet=ctx.createGain(),delay=ctx.createDelay(1),feedback=ctx.createGain(),sum=ctx.createGain();dry.gain.value=1-mix;wet.gain.value=mix;delay.delayTime.value=mapMusicAutomationValue('effectAmount',amount,effect.type);feedback.gain.value=mapMusicAutomationValue('effectFeedback',feedbackValue,effect.type);node.connect(dry);dry.connect(sum);node.connect(delay);delay.connect(wet);wet.connect(sum);delay.connect(feedback);feedback.connect(delay);node=sum;paramRegistry?.set(effect.id,{amount:delay.delayTime,mix:wet.gain,feedback:feedback.gain});} } node.connect(destination); };
  ...
  const exportWav = async () => {
    ...
    const trackChannels=new Map<string,{input:GainNode;volume:GainNode;panner:StereoPannerNode;sends:Map<string,GainNode>;effects:Map<string,{amount?:AudioParam;mix?:AudioParam;feedback?:AudioParam;resonance?:AudioParam}>}>();
    ...
    const ensureTrack=(track:MusicTrack)=>{const existing=trackChannels.get(track.id);if(existing)return existing;const input=offline.createGain(),volume=offline.createGain(),panner=offline.createStereoPanner(),sends=new Map<string,GainNode>(),effects=new Map<string,{amount?:AudioParam;mix?:AudioParam;feedback?:AudioParam;resonance?:AudioParam}>();volume.gain.value=track.volume;input.connect(volume);volume.connect(panner);connectTrackEffects(offline,panner,track,offline.destination,0,effects);panner.pan.value=track.pan;for(const send of track.sends??[]){const bus=project.returnBuses?.find(b=>b.id===send.busId);if(!send.enabled||!bus)continue;const sendGain=offline.createGain();sendGain.gain.value=send.level;panner.connect(sendGain);sendGain.connect(ensureReturn(bus).input);sends.set(send.busId,sendGain);}const channel={input,volume,panner,sends,effects};trackChannels.set(track.id,channel);return channel;};
    for(const track of audibleMusicTracks(project.tracks)){
      const channel=ensureTrack(track);
      for(const lane of (project.automationLanes??[]).filter(l=>l.trackId===track.id)){
        const effect=lane.targetId?track.effects?.find(item=>item.id===lane.targetId):undefined,handles=lane.targetId?channel.effects.get(lane.targetId):undefined,target=lane.parameter==='volume'?channel.volume.gain:lane.parameter==='pan'?channel.panner.pan:lane.parameter==='sendLevel'&&lane.targetId?channel.sends.get(lane.targetId)?.gain:lane.parameter==='effectMix'?handles?.mix:lane.parameter==='effectFeedback'?handles?.feedback:lane.parameter==='effectResonance'?handles?.resonance:lane.parameter==='effectAmount'?handles?.amount:undefined;if(!target)continue;
        const points=lane.points;for(let i=0;i<points.length;i++){const p=points[i],next=points[i+1]??p,t=p.step*stepDuration,value=mapMusicAutomationValue(lane.parameter,p.value,effect?.type);target.setValueAtTime(value,t);if(next!==p&&lane.interpolation!=='step'){const nt=next.step*stepDuration,nv=mapMusicAutomationValue(lane.parameter,next.value,effect?.type);if(lane.interpolation==='linear')target.linearRampToValueAtTime(nv,nt);else target.setValueCurveAtTime(new Float32Array([value,value+(nv-value)*.15625,value+(nv-value)*.5,value+(nv-value)*.84375,nv]),t,Math.max(.001,nt-t));}}
      }
