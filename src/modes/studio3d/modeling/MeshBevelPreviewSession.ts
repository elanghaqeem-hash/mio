...
export interface MeshBevelPreviewParameters { widthRatio:number; segments:number; profile:number; curvature:number }
...
export class MeshBevelPreviewSession {
  ...
  preview(parameters:MeshBevelPreviewParameters):MeshBevelPreviewResult{
    ...
    if(rail.closed){
      ...
      const refined=segmentBevelFaces(base.mesh,base.bevelFaceIds,parameters.segments,parameters.profile,parameters.curvature);
