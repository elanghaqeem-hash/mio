...
export const applyMeshTransformDelta=(mesh:MioMeshData,selection:MioMeshSelection,state:MeshTransformState,delta:[number,number,number]):MioMeshData=>{
 ...
 if(state.kind==='translate'){ const {translateMeshSelection}=require('./MeshOperations') as typeof import('./MeshOperations'); return translateMeshSelection(mesh,selection,d); }
