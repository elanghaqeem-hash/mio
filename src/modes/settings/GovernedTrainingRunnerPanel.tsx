...
import { Cpu, FolderOpen, Play, RefreshCw, ShieldCheck, Square } from 'lucide-react';
import {
...
export const GovernedTrainingRunnerPanel: React.FC = () => {
  ...
  const refreshHistory = async () => {
    ...
    try {
      ...
      if (running) {
        setJob(running);
        setWorkspace((current) => current ?? { id: running.workspaceId, name: 'Recovered training workspace' });
      }
    } catch (error) {
          ...
          if (terminal(next.state)) {
            ...
            if (workspace?.id === next.workspaceId) {
              try {
                ...
                // A terminal job has already released process use; manual revoke remains available if this best-effort cleanup fails.
              }
            }
          }
        } catch (error) {
  ...
  const authorizeWorkspace = async () => {
    ...
    setMessage(null);
    try {
      if (workspace && !job?.state?.includes('RUNNING')) {
  ...
  const start = async () => {
    ...
    try {
      ...
      setMessage(mode === 'DRY_RUN'
        ...
        : 'Governed local training started. Completion remains TRAINED_NOT_EVALUATED and still requires TP-0.58 handoff, MioBench, review, promotion, post-promotion integrity, and activation gates.');
    } catch (error) {
  ...
  if (!desktopAvailable) {
    return (
      <div className="bg-[#0d121d] p-4 rounded-xl border border-gray-800 space-y-2">
        ...
        <p className="text-[10px] text-gray-500">Desktop/Electron only. Browser deployments cannot spawn Python, access a local training workspace, or run TP-0.46 training.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#0d121d] p-4 rounded-xl border border-gray-800 space-y-4">
      ...
      <div className="rounded border border-cyan-500/20 bg-cyan-950/10 p-3 text-[10px] leading-relaxed text-gray-400">
        ...
        Mio can only start the bundled <code>training/train_mio_lora.py</code> runner with a whitelisted Python launcher. No shell string or arbitrary executable is accepted. The capability declares no network access and the runner receives offline Hugging Face/Transformers/Datasets flags; this is application-level network minimization, not an operating-system firewall.
      </div>
      ...
      {job && (
        <div className="rounded border border-gray-800 bg-[#080b12] p-3 space-y-2">
          <div className="grid gap-1 text-[10px] text-gray-400 md:grid-cols-2">
            ...
            {job.resultFileRelativePath && <span className="md:col-span-2">TP-0.58 result input: <code className="text-cyan-300">{job.resultFileRelativePath}</code></span>}
          </div>
          ...
          {job.state === 'SUCCEEDED' && job.mode === 'TRAIN' && <p className="text-[10px] text-emerald-300">Training completed only as TRAINED_NOT_EVALUATED. Package the exact bundle + result into TP-0.58 handoff before Candidate Lab evaluation; no promotion or activation occurred.</p>}
        </div>
      )}
