...
export const ProjectOverviewView: React.FC = () => {
  ...
  const runCompositePipeline = async () => {
    ...
    const steps = CreativeOrchestrator.planCreativePipeline('Create a 3d futuristic drone, animate locomotion hovering, draw a concept illustration, retouch a photo, synthesize laser sound effects, compose cyberpunk background music, design a technical poster, and create 2D motion graphics');
  ...
  const getAssetIcon = (type: ProjectAsset['type']) => {
    switch (type) {
      ...
      case 'motion-2d': return <Film size={14} className="text-fuchsia-400" />;
      case 'drawing': return <Palette size={14} className="text-pink-400" />;
      case 'graphic': return <Palette size={14} className="text-amber-400" />;
      case 'photo': return <Palette size={14} className="text-sky-400" />;
