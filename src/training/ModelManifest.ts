...
export function validateModelManifest(manifest: MioModelManifest): ModelManifestValidation {
  ...
  if (manifest.promotion) {
    ...
    if (manifest.promotion.artifactBindingEvidenceId !== undefined && !manifest.promotion.artifactBindingEvidenceId.trim()) errors.push('Promotion provenance artifactBindingEvidenceId is invalid');
