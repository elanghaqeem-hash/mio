...
export interface ContextMemoryItem {
  id: string; layer: ContextMemoryLayer; content: string; source: string; trust: ContextMemoryTrust;
  createdAt: number; updatedAt: number; projectId?: string; sessionId?: string; taskId?: string;
  expiresAt?: number; approvedByUser?: boolean;
}

interface PersistedProjectMemoryState { projectId: string; items: ContextMemoryItem[]; }
export interface ProjectMemoryInput { projectId: string; content: string; source: string; trust: ContextMemoryTrust; approvedByUser: boolean; }
...
export class MemoryContextManager {
  ...
  public static addWorking(taskId: string, content: string, source = 'runtime', ttlMs = 30 * 60 * 1000): ContextMemoryItem | null {
    ...
    const item: ContextMemoryItem = {
      id: `working_${now}_${Math.random().toString(36).slice(2, 7)}`, layer: 'WORKING', content: normalized,
      source, trust: 'SYSTEM_DERIVED', taskId, createdAt: now, updatedAt: now,
      expiresAt: now + Math.max(1000, Math.min(ttlMs, 24 * 60 * 60 * 1000)),
    };
    this.working.set(taskId, [...(this.working.get(taskId) ?? []), item]); return { ...item };
  }
  ...
  public static getConversation(sessionId: string): ContextMemoryItem[] { return (this.conversations.get(sessionId) ?? []).map((item) => ({ ...item })); }
  ...
  public static async addProjectMemory(input: ProjectMemoryInput): Promise<ContextMemoryItem | null> {
    const normalized = input.content.trim(); if (!input.projectId || !normalized) return null;
    if (input.trust === 'EXTERNAL_UNTRUSTED' || !input.approvedByUser) {
      eventBus.emit('MEMORY_POLICY_EVENT', {
        decision: 'PROJECT_MEMORY_DENIED', projectId: input.projectId,
        reason: input.trust === 'EXTERNAL_UNTRUSTED' ? 'External/untrusted content cannot be promoted directly to project memory.' : 'Project memory requires explicit user approval.',
      });
      return null;
    }
    ...
    const item: ContextMemoryItem = {
      ...
      source: input.source, trust: input.trust, projectId: input.projectId, approvedByUser: true, createdAt: now, updatedAt: now,
    };
    const next = [...(this.projectMemory.get(input.projectId) ?? []), item]; this.projectMemory.set(input.projectId, next);
    await this.flushProject(input.projectId); eventBus.emit('PROJECT_MEMORY_UPDATED', { projectId: input.projectId, count: next.length }); return { ...item };
  }
  ...
  public static proposeProjectMemoryForLongTerm(projectId: string, itemId: string): MemoryWriteResult | null {
    const item = (this.projectMemory.get(projectId) ?? []).find((entry) => entry.id === itemId); if (!item || !item.approvedByUser) return null;
    return MioMemoryManager.proposeMemory({
      category: 'PROJECT_CONTEXT', content: item.content, confidence: item.trust === 'PROJECT_VERIFIED' ? 0.95 : 0.85,
      source: `project-approved:${projectId}:${item.source}`, permissionLevel: 'L1_SUGGEST',
    });
  }
  ...
  public static purgeExpired(now = Date.now()): void {
    for (const [taskId, items] of this.working.entries()) {
      ...
      if (active.length) this.working.set(taskId, active); else this.working.delete(taskId);
    }
  }
  public static resetRuntimeLayers(): void { this.working.clear(); this.conversations.clear(); }

  private static async flushProject(projectId: string): Promise<void> {
    await this.storage.set<PersistedProjectMemoryState>('memory', projectStorageKey(projectId), { projectId, items: this.projectMemory.get(projectId) ?? [] });
  }
}
