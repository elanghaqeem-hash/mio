...
export async function runMioTestSuite() {
  ...
  const persistenceProject = ProjectManager.createProject('TP 0.19 Persistence Test', 'Storage abstraction validation');
  ...
  ProjectManager.reviewKnowledgeSource(governedDocument.id, 'VERIFIED', 'Reviewed during TP 0.19 test', Date.now() + 86_400_000);
  ProjectManager.setKnowledgeSourcePriority(governedDocument.id, 'PRIMARY');
  ...
  assert(persistedGovernance?.priority === 'PRIMARY', 'Knowledge source priority survives project storage persistence round-trip');
  ...
  const priorityEvent = persistedProject?.knowledgeGovernance.history.find((event) => event.assetId === governedDocument.id && event.action === 'PRIORITY_CHANGED');
  assert(priorityEvent?.priority === 'PRIMARY', 'Knowledge source priority decision persists in governance provenance history');
