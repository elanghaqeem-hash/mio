...
export async function runMeshTransformTransactionTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('selection pivot is centroid of selected vertices',()=>{const p=meshSelectionPivot(mesh,selection);assert(!!p&&p[0]===-0.5&&p[1]===-0.5&&p[2]===-0.5,'pivot should equal selected centroid')}));
