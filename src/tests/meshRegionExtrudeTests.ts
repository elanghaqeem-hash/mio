...
export async function runMeshRegionExtrudeTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('single-face region extrude creates one cap and four sides',()=>{const r=extrudeMeshRegion(createCubeMesh(),['f_front'],1);assert(r.capFaceIds.length===1,'expected one cap');assert(r.createdSideFaceIds.length===4,'expected four boundary sides');const firstSide=r.mesh.faces.find(face=>face.id===r.createdSideFaceIds[0]);assert(firstSide?.vertexIds[0]==='v4'&&firstSide?.vertexIds[1]==='v5','boundary side must preserve source face winding');assert(validateMeshTopology(r.mesh).valid,'result must remain valid')}));
 ...
 results.push(await test('negative distance supports inward region extrusion',()=>{const r=extrudeMeshRegion(createCubeMesh(),['f_front'],-0.25);const cap=r.mesh.faces.find(face=>face.id===r.capFaceIds[0]);const z=cap?.vertexIds.map(id=>r.mesh.vertices.find(v=>v.id===id)?.position[2] ?? NaN);assert(z?.every(value=>Math.abs(value-0.25)<1e-9),'front cap should move inward from z 0.5 to 0.25');assert(validateMeshTopology(r.mesh).valid,'inward result must remain valid')}));
