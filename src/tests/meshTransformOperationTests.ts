...
export async function runMeshTransformOperationTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('selection rotation keeps pivot fixed',()=>{const next=rotateMeshSelection(mesh,selection,[0,Math.PI/2,0]);const p=next.vertices.find(v=>v.id==='v0')?.position;assert(!!p&&Math.abs(p[0]+0.5)<1e-6&&Math.abs(p[2]-0.5)<1e-6,'rotation should occur around selection pivot')}));
 results.push(await test('selection scale keeps pivot fixed',()=>{const next=scaleMeshSelection(mesh,selection,[2,1,1]);const p=next.vertices.find(v=>v.id==='v0')?.position;assert(!!p&&p[0]===-0.5&&p[1]===-0.5,'pivot-centered scaling should preserve pivot-axis coordinates')}));
