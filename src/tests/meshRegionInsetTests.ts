...
export async function runMeshRegionInsetTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('region inset preserves internal shared topology',()=>{const r=insetMeshRegion(twoQuadPlane(),['left','right'],0.5);const left=r.mesh.faces.find(face=>face.id===r.insetFaceIds[0]);const right=r.mesh.faces.find(face=>face.id===r.insetFaceIds[1]);assert(!!left&&!!right,'inset faces required');const shared=left.vertexIds.filter(id=>right.vertexIds.includes(id));assert(shared.length===2,'inset caps must share the duplicated internal edge')}));
