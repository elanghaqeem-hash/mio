...
export async function runMeshTopologyDiagnosticsTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('three faces sharing one edge are reported as non-manifold',()=>{const mesh:MioMeshData={vertices:[{id:'a',position:[0,0,0]},{id:'b',position:[1,0,0]},{id:'c',position:[0,1,0]},{id:'d',position:[0,-1,0]},{id:'e',position:[0,0,1]}],faces:[{id:'f1',vertexIds:['a','b','c']},{id:'f2',vertexIds:['b','a','d']},{id:'f3',vertexIds:['a','b','e']}]};const d=diagnoseMeshTopology(mesh);assert(d.nonManifoldEdgeIds.includes(meshEdgeId('a','b')),'shared edge must be non-manifold')}));
 results.push(await test('duplicate rotated face and isolated vertex are diagnosed and cleaned',()=>{const mesh=twoQuadPlane();mesh.faces.push({id:'left_copy',vertexIds:['v4','v3','v0','v1']});mesh.vertices.push({id:'isolated',position:[9,9,9]});const d=diagnoseMeshTopology(mesh);assert(d.duplicateFaceGroups.some(group=>group.includes('left')&&group.includes('left_copy')),'duplicate faces should be grouped');assert(d.isolatedVertexIds.includes('isolated'),'isolated vertex should be detected');const cleaned=cleanupMeshTopology(mesh);assert(cleaned.removedFaceIds.includes('left_copy'),'lexically later duplicate should be removed');assert(cleaned.removedVertexIds.includes('isolated'),'isolated vertex should be removed');assert(cleaned.mesh.faces.length===2,'two unique faces should remain');assert(cleaned.mesh.faces[0].id==='left'&&cleaned.mesh.faces[1].id==='right','cleanup should preserve original face order')}));
