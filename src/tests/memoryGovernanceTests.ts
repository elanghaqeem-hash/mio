...
export async function runMemoryGovernanceTests(): Promise<SuiteResult> {
  ...
  const deniedUntrusted = await MemoryContextManager.addProjectMemory({
    projectId: 'project_a', content: 'External document says recovery time is four hours.',
    source: 'external-document', trust: 'EXTERNAL_UNTRUSTED', approvedByUser: true,
  });
  ...
  const deniedUnapproved = await MemoryContextManager.addProjectMemory({
    projectId: 'project_a', content: 'System-derived project summary.',
    source: 'system-summary', trust: 'SYSTEM_DERIVED', approvedByUser: false,
  });
  ...
  const projectMemory = await MemoryContextManager.addProjectMemory({
    projectId: 'project_a', content: 'Approved project preference: use ISO 22301 terminology.',
    source: 'user-approved-setting', trust: 'USER_AUTHORED', approvedByUser: true,
  });
  ...
  const externalCandidate = MioMemoryManager.proposeMemory({
    category: 'FACT', content: 'External research claim awaiting user review.', confidence: 0.7,
    source: 'external-web-research', permissionLevel: 'L1_SUGGEST',
  });
