...
export async function runMemoryGovernanceTests(): Promise<SuiteResult> {
  let passed = 0;
  let total = 0;
  const check = (condition: boolean, label: string) => {
    total += 1;
    if (!condition) throw new Error(`MemoryGovernance test failed: ${label}`);
    passed += 1;
  ...
  check(MemoryContextManager.getWorking('task_1').length === 1, 'Working memory is available during the active task context');
  ...
  const deniedUntrusted = await MemoryContextManager.addProjectMemory({
    projectId: 'project_a',
    content: 'External document says recovery time is four hours.',
  ...
  const deniedUnapproved = await MemoryContextManager.addProjectMemory({
    projectId: 'project_a',
    content: 'System-derived project summary.',
  ...
  const projectMemory = await MemoryContextManager.addProjectMemory({
    projectId: 'project_a',
    content: 'Approved project preference: use ISO 22301 terminology.',
    source: 'user-approved-setting',
    trust: 'USER_AUTHORED',
    approvedByUser: true,
  });
  ...
  check(MemoryContextManager.getProjectMemory('project_a').some((item) => item.id === projectMemory?.id), 'Approved project memory survives storage persistence round-trip');

  const promoted = projectMemory ? MemoryContextManager.proposeProjectMemoryForLongTerm('project_a', projectMemory.id) : null;
  check(promoted?.status === 'SAVED', 'Explicitly approved project memory can be proposed to long-term memory through MemoryManager policy');

  const externalCandidate = MioMemoryManager.proposeMemory({
    category: 'FACT',
    content: 'External research claim awaiting user review.',
    confidence: 0.7,
  ...
  check(persisted?.pendingCandidates?.some((candidate) => candidate.id === (externalCandidate.status === 'REVIEW_REQUIRED' ? externalCandidate.candidateId : '')) === true, 'Long-term memory review queue is persisted across reload boundaries');

  MemoryContextManager.resetRuntimeLayers();
  check(MemoryContextManager.getWorking('task_1').length === 0 && MemoryContextManager.getConversation('session_1').length === 0, 'Runtime memory reset clears ephemeral layers without deleting project memory');
  check(MemoryContextManager.getProjectMemory('project_a').length === 1, 'Runtime memory reset preserves persistent project memory');

  MioMemoryManager.clearAll();
  await MioMemoryManager.flush();
  MioMemoryManager.setStorageProvider(defaultStorageProvider);
