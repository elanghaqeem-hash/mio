...
export async function runPhotoDomainContractTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('photo tile engine maps edge regions and evicts deterministically',()=>{const tiles=photoTilesForRegion('doc',1,{width:600,height:300},{x:500,y:250,width:100,height:50},256);assert(tiles.length===2,'edge region should map to two tiles');assert(tiles[1].bounds.width===88&&tiles[1].bounds.height===44,'edge tile bounds incorrect');const cache=new PhotoTileCache<string>(2,10);cache.set('a','A',4);cache.set('b','B',4);cache.get('a');cache.set('c','C',4);assert(cache.has('a')&&!cache.has('b')&&cache.has('c'),'LRU eviction incorrect');assert(cache.bytes===8,'byte accounting incorrect');}));
