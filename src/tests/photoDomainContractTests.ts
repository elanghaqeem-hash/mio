...
export async function runPhotoDomainContractTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('photo cpu compositor preserves layer order opacity and supported blend modes',()=>{const bg=createPhotoRenderSurface(1,1).pixels,fg=createPhotoRenderSurface(1,1).pixels;bg.data.set([0,0,255,255]);fg.data.set([255,0,0,255]);compositePhotoLayer(bg,fg,0.5,'normal');assert(bg.data[0]===128&&bg.data[2]===127,'layer opacity compositing incorrect');const out=compositePhotoLayers(1,1,[{pixels:createPhotoRenderSurface(1,1).pixels,opacity:1,blendMode:'normal',visible:true},{pixels:fg,opacity:1,blendMode:'multiply',visible:true}]);assert(out.pixels.data[0]===0,'multiply blend baseline incorrect');}));
