...
export async function runPhotoDomainContractTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('photo transform sampler handles translation flip pivot and transparent edges',()=>{const src=createPhotoRenderSurface(2,1).pixels;src.data.set([255,0,0,255,0,0,255,255]);assert(samplePhotoPixel(src,-1,0,'nearest')[3]===0,'out-of-bounds sampling must be transparent');const t={x:0,y:0,scaleX:-1,scaleY:1,rotation:0,skewX:0,skewY:0};const p=inverseMapPhotoPoint(0,0,t,{x:0.5,y:0});assert(Math.abs(p.x-1)<=1e-8,'negative scale inverse mapping incorrect');const out=transformPhotoBuffer(src,2,1,t,'nearest',{x:0.5,y:0});assert(out.data[2]===255&&out.data[4]===255,'horizontal flip output incorrect');}));
 results.push(await test('photo cpu compositor evaluates deterministic mask alpha',()=>{assert(Math.round(applyPhotoMaskAlpha(1,0.5,1,false)*255)===128,'mask alpha incorrect');assert(Math.round(applyPhotoMaskAlpha(1,0.5,1,true)*255)===128,'inverted mask alpha incorrect');const src=createPhotoRenderSurface(1,1).pixels;src.data.set([255,0,0,255]);const out=compositePhotoLayers(1,1,[{pixels:src,opacity:1,blendMode:'normal',visible:true,mask:{data:new Uint8ClampedArray([128]),width:1,height:1}}]);assert(out.pixels.data[3]===128,'masked alpha was not applied');}));
 results.push(await test('photo cpu compositor preserves layer order opacity and supported blend modes',()=>{const bg=createPhotoRenderSurface(1,1).pixels,fg=createPhotoRenderSurface(1,1).pixels;bg.data.set([0,0,255,255]);fg.data.set([255,0,0,255]);compositePhotoLayer(bg,fg,0.5,'normal');assert(Math.abs(bg.data[0]-128)<=1&&Math.abs(bg.data[2]-128)<=1&&bg.data[3]===255,'layer opacity compositing incorrect');const out=compositePhotoLayers(1,1,[{pixels:createPhotoRenderSurface(1,1).pixels,opacity:1,blendMode:'normal',visible:true},{pixels:fg,opacity:1,blendMode:'multiply',visible:true}]);assert(out.pixels.data[0]===0,'multiply blend baseline incorrect');}));
 results.push(await test('photo cpu reference source-over blending is deterministic',()=>{const dst=createPhotoRenderSurface(1,1).pixels,src=createPhotoRenderSurface(1,1).pixels;dst.data.set([0,0,255,255]);src.data.set([255,0,0,128]);blendSourceOver(dst,src);assert(dst.data[0]===128&&dst.data[1]===0&&dst.data[2]===127&&dst.data[3]===255,'source-over RGBA result incorrect');}));
 results.push(await test('photo tile scheduler skips cache hits and cancels stale generations',()=>{const tiles=photoTilesForRegion('doc',1,{width:512,height:512},undefined,256);const planned=planPhotoTileQueue(tiles,{x:0,y:0,width:256,height:256},2,{cachedKeys:new Set([tiles[0].cacheKey]),maxConcurrent:2});assert(!planned.some(t=>t.cacheHit),'cache hit remained scheduled');const q=new PhotoTileQueue(1);q.enqueue(planned);const task=q.next(2);assert(task?.generation===2,'current generation not scheduled');q.complete();const staleQueue=new PhotoTileQueue(1);staleQueue.enqueue(planned.map(t=>({...t,generation:1})));staleQueue.cancelStale(2);assert(staleQueue.pendingCount===0,'stale tasks were not cancelled');const newerQueue=new PhotoTileQueue(1);newerQueue.enqueue(planned.map(t=>({...t,generation:2})));newerQueue.cancelStale(3);assert(newerQueue.pendingCount===0,'older generation was not cancelled');}));
