...
export async function runMotionM2CoreTests():Promise<{passed:number;total:number}>{
 ...
 run("playback preserves sub-frame elapsed time",()=>{let state={frame:0,playing:true,loop:false,rate:1};for(let i=0;i<60;i++)state=stepMotionPlayback(composition,state,1/60);assert(state.frame===30,"60 half-frame ticks at 30fps must advance exactly 30 frames")});
