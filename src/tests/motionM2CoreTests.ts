...
import type { MotionComposition, MotionDocument } from "../creative/motion/model";
...
export async function runMotionM2CoreTests():Promise<{passed:number;total:number}>{
 ...
 run("playback preserves sub-frame elapsed time",()=>{let state={frame:0,playing:true,loop:false,rate:1};for(let i=0;i<60;i++)state=stepMotionPlayback(comp,state,1/60);assert(state.frame===30,"60 half-frame ticks at 30fps must advance exactly 30 frames")});
 ...
 run("multi-key move is bounded and deterministic",()=>{const n=moveSelectedKeyframes([{id:"a",frame:2},{id:"b",frame:8}],["a","b"],-5,0,20);assert(n[0].frame===0&&n[1].frame===6,"bounded group move must preserve spacing")});
 run("multi-key move avoids occupied unselected frames",()=>{const n=moveSelectedKeyframes([{id:"a",frame:2},{id:"b",frame:5},{id:"c",frame:8}],["a","b"],3,0,20);assert(n.find(k=>k.id==="a")?.frame===4&&n.find(k=>k.id==="b")?.frame===7,"collision-safe delta failed");assert(n.find(k=>k.id==="c")?.frame===8,"unselected key moved")});
 ...
 run("workspace controller persists timeline selection",()=>{const layer={id:"l",name:"L",type:"shape" as const,inFrame:0,outFrame:29,enabled:true,transform:{position:{id:"p",property:"position",defaultValue:[0,0] as const,keyframes:[]},scale:{id:"s",property:"scale",defaultValue:[1,1] as const,keyframes:[]},rotation:{id:"r",property:"rotation",defaultValue:0,keyframes:[]},opacity:{id:"o",property:"opacity",defaultValue:1,keyframes:[]}}};const document:MotionDocument={schemaVersion:1,id:"d",activeCompositionId:"c",compositions:[{id:"c",name:"C",width:100,height:100,fps:30,durationFrames:30,workArea:[0,29],layers:[layer]}]};const controller=new MotionWorkspaceController(document);controller.setSelection({layerIds:["l"],keyframeIds:["k1","k2"]});const selection=controller.snapshot().selection;assert(selection.layerIds[0]==="l"&&selection.keyframeIds.join(",")==="k1,k2","controller selection drifted")});
