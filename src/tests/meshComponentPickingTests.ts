...
export async function runMeshComponentPickingTests():Promise<{passed:number;total:number}>{
 ...
 results.push(await test('edge picker resolves a projected edge vicinity',()=>{const picked=pickMeshEdgeScreenSpace(mesh,object,camera,new Vector2(500,500),1000,1000,20);assert(!!picked,'expected nearby edge')}));
