...
export class MioLocalProvider implements ModelProvider {
  ...
  public async generate(request: ModelRequest, signal?: AbortSignal): Promise<ModelResponse> {
    ...
    const messages: ModelMessage[] = [
      { role: 'system', content: this.buildAgentPolicy() },
    ...
    return this.toResponse(finalText, finalLocal.model, finalLocal.finishReason, inputTokens, outputTokens, attemptedWebSearch, citations);
  }
  ...
  private async searchWeb(query: string, signal?: AbortSignal): Promise<WebEvidence[]> {
    ...
    return (payload.results ?? []).flatMap((item) => {
      ...
      if (!url) return [];
      return [{ title: cleanText(item.title, 240) || 'Untitled source', url, excerpt: cleanText(item.excerpt, 1_500) }];
    }).slice(0, MAX_EVIDENCE_ITEMS);
  }

  private formatEvidence(query: string, evidence: WebEvidence[]): string {
    if (evidence.length === 0) return `[MIO_WEB_EVIDENCE trust="UNTRUSTED" query=${JSON.stringify(query)}]\nNo usable web results were returned. Do not claim the query was verified.\n[/MIO_WEB_EVIDENCE]`;
    return [
