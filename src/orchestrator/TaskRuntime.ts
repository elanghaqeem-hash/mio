...
class TaskRuntimeController {
  ...
  public create(plan: TaskPlan, prompt: string, projectId?: string, maxRetries: number = 1): RuntimeTask {
    ...
    const task: RuntimeTask = {
      ...
      progress: 0,
      steps: plan.steps.map((step): RuntimeTaskStep => ({
        ...step,
        status: 'PENDING',
      })),
      createdAt: now,
  ...
  public waitForPermission(taskId: string): RuntimeTask | undefined {
    ...
    task.updatedAt = Date.now();
    this.emitTaskEvent(taskId, 'WAITING_PERMISSION');
    this.publishSnapshot();
    return this.clone(task);
  }
  ...
  public pause(taskId: string): RuntimeTask | undefined {
    ...
    task.status = 'PAUSED';
    task.updatedAt = Date.now();
    this.emitTaskEvent(taskId, 'PAUSED');
    this.publishSnapshot();
    return this.clone(task);
  }
  ...
  public startStep(taskId: string, stepId: string): RuntimeTask | undefined {
    ...
    this.emitTaskEvent(taskId, 'STEP_STARTED', step.label);
    this.publishSnapshot();
    return this.clone(task);
  }
  ...
  public completeStep(taskId: string, stepId: string): RuntimeTask | undefined {
    ...
    task.updatedAt = Date.now();
    this.recalculateProgress(task);
    this.emitTaskEvent(taskId, 'STEP_COMPLETED', step.label);
    this.publishSnapshot();
    return this.clone(task);
  }
  ...
  public scheduleRetry(taskId: string): RuntimeTask | undefined {
    const task = this.getMutable(taskId);
    if (!task || task.status !== 'FAILED' || task.retryCount >= task.maxRetries || emergencyStop.isEmergencyStopped()) {
    ...
    this.recalculateProgress(task);
    this.emitTaskEvent(taskId, 'RETRY_SCHEDULED', `Retry ${task.retryCount}/${task.maxRetries}`);
    this.publishSnapshot();
    return this.clone(task);
  }
  ...
  public cancelAllActive(reason: string): void {
    Array.from(this.tasks.values())
      ...
      .forEach((task) => this.cancel(task.id, reason));
  }
  ...
  public get(taskId: string): RuntimeTask | undefined {
  ...
  public snapshot(): TaskRuntimeSnapshot {
    ...
    return {
      tasks,
      activeTaskIds: tasks.filter((task) => !TERMINAL_STATES.has(task.status)).map((task) => task.id),
      updatedAt: Date.now(),
    };
  }
  ...
  private getMutable(taskId: string): RuntimeTask | undefined {
    return this.tasks.get(taskId);
  }

  private recalculateProgress(task: RuntimeTask): void {
    if (task.steps.length === 0) {
      task.progress = TERMINAL_STATES.has(task.status) ? 100 : 0;
      return;
    }
  ...
  private emitTaskEvent(taskId: string, type: TaskRuntimeEvent['type'], details?: string): void {
    ...
    eventBus.emit('ACTIVITY_LOG', {
      ...
      message: `Task ${taskId}: ${type}${details ? ` — ${details}` : ''}`,
      mode: 'PROJECT',
    });
  }
  ...
  private clone(task: RuntimeTask): RuntimeTask {
    return {
      ...task,
