...
export class TaskScheduler {
  ...
  public snapshot(): TaskSchedulerSnapshot {
    return { queuedTaskIds: this.getQueuedTaskIds(), activeTaskIds: this.getActiveTaskIds(), concurrencyLimit: this.concurrencyLimit, updatedAt: Date.now() };
  }
  ...
  public async waitUntilRunnable(taskId: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      const unsubscribe = eventBus.on<TaskRuntimeSnapshot>('TASK_RUNTIME_SNAPSHOT', (snapshot) => {
        ...
        if (!task || task.status === 'CANCELLED' || task.status === 'FAILED') { unsubscribe(); reject(new Error(task?.error ?? 'Task stopped while paused')); return; }
        if (task.status !== 'PAUSED') { unsubscribe(); resolve(); }
      });
    });
  }
  ...
  private enqueue<T>(taskId: string, runner: QueueRunner<T>): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      this.queue.push({ taskId, runner: runner as QueueRunner<unknown>, resolve: (value) => resolve(value as T), reject, enqueuedAt: Date.now() });
  ...
  private pump(): void {
    ...
    for (let index = 0; index < this.queue.length && this.active.size < this.concurrencyLimit;) {
      ...
      if (!runtimeTask || runtimeTask.status === 'CANCELLED') { this.queue.splice(index, 1); entry.reject(new Error('Task is no longer executable')); changed = true; continue; }
      if (runtimeTask.status === 'PAUSED' || !taskRuntime.dependenciesSatisfied(entry.taskId)) { index += 1; continue; }
      ...
      changed = true; void this.runEntry(entry);
    }
    if (changed) this.publish();
  }

  private async runEntry(entry: QueueEntry): Promise<void> {
    try { entry.resolve(await entry.runner()); }
    catch (error) {
      ...
      entry.reject(error);
    } finally { this.active.delete(entry.taskId); this.publish(); this.pump(); }
  }

  private rejectQueuedTask(taskId: string, error: Error): void {
    const index = this.queue.findIndex((entry) => entry.taskId === taskId);
    if (index < 0) return;
    const [entry] = this.queue.splice(index, 1); entry.reject(error); this.publish();
  }

  private rejectAllQueued(error: Error): void {
    const pending = this.queue.splice(0, this.queue.length); pending.forEach((entry) => entry.reject(error)); this.publish();
  }
