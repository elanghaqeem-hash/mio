...
export interface SFXScheduledSample { source:AudioBufferSourceNode; gain:GainNode; pan:StereoPannerNode; stopTime:number; }
...
export const scheduleSFXSampleRegion=(context:BaseAudioContext,decoded:SFXDecodedSample,region:SFXSampleRegion,destination:AudioNode,when=context.currentTime):SFXScheduledSample=>{
 ...
 const duration=regionTimelineDuration(r),rate=effectivePlaybackRate(r),fadeIn=r.fadeIn/rate,fadeOut=r.fadeOut/rate;
 ...
 const offset=r.reverse?decoded.buffer.duration-r.sourceEnd:r.sourceStart;source.start(when,Math.max(0,offset));if(!r.loop)source.stop(when+duration);
 return {source,gain,pan,stopTime:when+duration};
};
