...
export const schedulePhotoTiles=(tiles:PhotoTile[],viewport:PhotoViewport,generation:number,preload=512):PhotoScheduledTile[]=>{
 ...
 return tiles.map(tile=>{const visible=intersects(tile.bounds,viewport),near=intersects(tile.bounds,expanded);const priority:PhotoTilePriority=visible?'interactive':near?'foreground':'background';return {tile,priority,distance:distance(tile.bounds,viewport),generation,cacheHit:false,score:(priority==='interactive'?0:priority==='foreground'?100000:200000)+distance(tile.bounds,viewport)};}).sort((a,b)=>{const rank=(p:PhotoTilePriority)=>p==='interactive'?0:p==='foreground'?1:2;return rank(a.priority)-rank(b.priority)||a.distance-b.distance||a.tile.id.localeCompare(b.tile.id);});
};
