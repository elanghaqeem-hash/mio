...
export const compilePhotoCommand=(document:CreativeDocument,command:PhotoCommand):CreativeCommand=>{
 ...
 switch(command.type){
  ...
  case 'photo.layer.clip':{const node=document.nodes[command.nodeId];if(!node)throw new Error(`Node ${command.nodeId} does not exist.`);if(command.targetId&&!document.nodes[command.targetId])throw new Error(`Clipping target ${command.targetId} does not exist.`);if(command.targetId===command.nodeId)throw new Error('A layer cannot clip to itself.');return {type:'node.update',nodeId:command.nodeId,changes:{properties:{...node.properties,clippingTargetId:command.targetId}}};}
 }
};
