...
export const inverseMapPhotoPoint=(x:number,y:number,t:PhotoTransform):{x:number;y:number}=>{const sx=t.scaleX||1,sy=t.scaleY||1,rad=-t.rotation*Math.PI/180,dx=x-t.x,dy=y-t.y,rx=dx*Math.cos(rad)-dy*Math.sin(rad),ry=dx*Math.sin(rad)+dy*Math.cos(rad);return {x:rx/sx,y:ry/sy};};
export const transformPhotoBuffer=(src:PhotoPixelBuffer,width:number,height:number,t:PhotoTransform,mode:PhotoSamplingMode='bilinear'):PhotoPixelBuffer=>{const data=new Uint8ClampedArray(width*height*4);for(let y=0;y<height;y++)for(let x=0;x<width;x++){const p=inverseMapPhotoPoint(x,y,t),v=samplePhotoPixel(src,p.x,p.y,mode),i=(y*width+x)*4;data.set(v,i);}return {width,height,data};};
