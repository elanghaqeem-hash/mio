...
export interface GraphicPathSegment { index:number; from:GraphicPathPoint; to:GraphicPathPoint; closing:boolean; }
export const getGraphicPathSegments=(path:GraphicPath):GraphicPathSegment[]=>{const segments:GraphicPathSegment[]=[];for(let index=0;index<path.points.length-1;index++)segments.push({index,from:path.points[index],to:path.points[index+1],closing:false});if(path.closed&&path.points.length>2)segments.push({index:path.points.length-1,from:path.points[path.points.length-1],to:path.points[0],closing:true});return segments;};
export const evaluateGraphicPathSegment=(segment:GraphicPathSegment,t:number):Point2D=>{const u=Math.max(0,Math.min(1,t)),a=segment.from,b=segment.to,p0={x:a.x,y:a.y},p1=a.outX!==undefined&&a.outY!==undefined?{x:a.outX,y:a.outY}:p0,p3={x:b.x,y:b.y},p2=b.inX!==undefined&&b.inY!==undefined?{x:b.inX,y:b.inY}:p3,q=1-u;return{x:q*q*q*p0.x+3*q*q*u*p1.x+3*q*u*u*p2.x+u*u*u*p3.x,y:q*q*q*p0.y+3*q*q*u*p1.y+3*q*u*u*p2.y+u*u*u*p3.y};};
const lerpGraphicPoint=(a:Point2D,b:Point2D,t:number):Point2D=>({x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t});
export const splitGraphicPathSegment=(segment:GraphicPathSegment,t:number,newPointId:string):{from:GraphicPathPoint;point:GraphicPathPoint;to:GraphicPathPoint}=>{const u=Math.max(0.0001,Math.min(0.9999,t)),p0={x:segment.from.x,y:segment.from.y},p1=segment.from.outX!==undefined&&segment.from.outY!==undefined?{x:segment.from.outX,y:segment.from.outY}:p0,p3={x:segment.to.x,y:segment.to.y},p2=segment.to.inX!==undefined&&segment.to.inY!==undefined?{x:segment.to.inX,y:segment.to.inY}:p3,a=lerpGraphicPoint(p0,p1,u),b=lerpGraphicPoint(p1,p2,u),d=lerpGraphicPoint(p2,p3,u),e=lerpGraphicPoint(a,b,u),g=lerpGraphicPoint(b,d,u),m=lerpGraphicPoint(e,g,u);return{from:{...segment.from,outX:a.x,outY:a.y},point:{id:newPointId,x:m.x,y:m.y,inX:e.x,inY:e.y,outX:g.x,outY:g.y,nodeType:'smooth'},to:{...segment.to,inX:d.x,inY:d.y}};};
export const nearestGraphicPathSegmentPoint=(path:GraphicPath,point:Point2D,samples=24):{segment:GraphicPathSegment;t:number;point:Point2D;distance:number}|null=>{let best:null|{segment:GraphicPathSegment;t:number;point:Point2D;distance:number}=null;for(const segment of getGraphicPathSegments(path))for(let i=0;i<=samples;i++){const t=i/samples,p=evaluateGraphicPathSegment(segment,t),distance=Math.hypot(point.x-p.x,point.y-p.y);if(!best||distance<best.distance)best={segment,t,point:p,distance};}return best;};
export const insertGraphicPathPointOnSegment=(document:MioGraphicDocument,layerId:string,segmentIndex:number,t:number):MioGraphicDocument=>{return {...document,layers:document.layers.map(layer=>{if(layer.id!==layerId||layer.locked||!layer.path)return layer;const segments=getGraphicPathSegments(layer.path),segment=segments[segmentIndex];if(!segment)return layer;const split=splitGraphicPathSegment(segment,t,`${layerId}_p${layer.path!.points.length}_s${segmentIndex}`),points=[...layer.path!.points];const fromIndex=points.findIndex(p=>p.id===segment.from.id),toIndex=points.findIndex(p=>p.id===segment.to.id);if(fromIndex<0||toIndex<0)return layer;points[fromIndex]=split.from;points[toIndex]=split.to;points.splice(fromIndex+1,0,split.point);return{...layer,path:{...layer.path,points}};})}};
