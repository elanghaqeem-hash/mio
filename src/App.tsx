...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubMode = eventBus.on('SWITCH_MODE', (mode: MioSystemMode) => { if (validModes.includes(mode)) setActiveMode(mode); });
    const unsubPerm = eventBus.on('REQUEST_DRY_RUN_PERMISSION', (req: DryRunRequest) => setDryRunRequest(req));
    const unsubDesktopNav = window.mioDesktop?.onNavigate((mode) => { if (validModes.includes(mode as MioSystemMode)) setActiveMode(mode as MioSystemMode); });
    return () => { unsubState(); unsubMode(); unsubPerm(); unsubDesktopNav?.(); };
  }, []);

  const renderActiveWorkspace = () => {
    switch (activeMode) {
      case 'CHAT': return <ChatStudioView />; case '3D': return <Studio3DView />; case 'ANIMATION': return <AnimationStudioView />; case 'GRAPHIC': return <GraphicStudioView />; case 'SFX': return <SFXStudioView />; case 'MUSIC': return <MusicStudioView />; case 'RESEARCH': return <ResearchStudioView />; case 'FILES': return <FileOrganizationView />; case 'MOTION': return <MotionTrackingView />; case 'SECURITY': return <SecurityDashboardView />; case 'PROJECT': return <ProjectOverviewView />; case 'SETTINGS': return <SettingsView />; default: return <ChatStudioView />;
    }
  };

  if (!bootReady) return <div className="h-screen w-screen bg-[#07090e] text-cyan-300 flex items-center justify-center font-mono text-xs">INITIALIZING SECURE MIO RUNTIME...</div>;

  return <div className="flex flex-col h-screen w-screen bg-[#07090e] text-slate-200 overflow-hidden font-sans select-none">
    <TopBar coreState={coreState} activeMode={activeMode} onOpenSecurity={() => setActiveMode('SECURITY')} />
    <div className="flex flex-1 overflow-hidden"><ModeNavigation activeMode={activeMode} onSelectMode={setActiveMode} /><main className="flex-1 h-full overflow-hidden relative flex flex-col">{renderActiveWorkspace()}</main><ContextPanel /></div>
